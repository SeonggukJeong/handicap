...
| **에디터 구조 재설계 (B13)** | 슬라이스 1 완료 — 세로 아웃라인 `FlowOutline`·그룹내 dnd-kit 드래그·디테일 1fr·변수 접기·YAML 양방향 모달·React Flow 제거(ADR-0044) | **YAML 가져오기/내보내기 file-I/O**(사용자 요청·빠른 후속; share→paste는 모달로 충족됨 → file 표면만, `downloadJson.ts`/`ScenarioImportPage.readText` 패턴 차용) **또는 슬라이스 2**(하단 흐름 가로 칩 다이어그램+test-run 결과 색/아이콘·칩→스텝 선택). 슬라이스 3(컨테이너 경계 넘는 re-parent dnd)은 그 다음 | roadmap.md §B13 |
