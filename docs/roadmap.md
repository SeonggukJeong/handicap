...
### D. 부하 모델·페이싱 설정 (영역 D) — **🚧 진행 중 (S-A ✅ · S-B ✅ · S-C ✅ 완료 · S-D 다음)**
...
- **4 하위 슬라이스, 전부 commit, 의존성 순서 1→4** (사용자 확정: 급한 것 없음):
  ...
  - **S-D 다단계 ramp (🚧 brainstorm 중, 2026-06-04)**: **범위 = open-loop RPS 곡선만** (사용자 확정). `stages: [{target=RPS, duration}]`로 S-C 고정 `target_rps`를 일반화 — `stages`는 `target_rps`+`duration_seconds`의 generic 대체(상호 배타, `max_in_flight` 유무로 open/closed 구분). **closed-loop stages(VU 곡선 + retire/ramp-down)는 (b)+(c)로 묶어 별도 미래 슬라이스 연기** — (b) 단독(올리기만, 회수 없음)은 비대칭 반쪽이라 양방향을 한 번에 완결하는 게 일관. UX = 빌트인 부하-모양 템플릿(일정/점증·유지/스파이크/계단/소크) + 라이브 곡선 미리보기 스파크라인 + 인라인 helper(`?`), stages 영역 국한. S-C 위에서 가장 가치.
