...
- **✅ Run 스케줄러 백엔드(34a+34b) 완료 (2026-06-06, ADR-0034)** — 예약(once)/반복(cron) 자동 발사. 컨트롤러 내장 `run_scheduler` 루프 + migration 0011(`schedules`/`schedule_events`) + `api/schedules.rs` CRUD/preview-next/events. **34c(UI) 잔여.** 엔진·워커·proto 무변경. 상세 §A7.
- **✅ A2 Parallel 노드 완료 (2026-06-06, ADR-0033)** — 동시 분기 fan-out(`join_all` wait-all + `{{branch.var}}` key-origin merge). 상세 §A2.
- **🎯 다음 우선순위**: 도출된 순서(load fidelity → reporting depth, §A2)의 **(2) 그룹/페이지 레이턴시**(동시 호출의 max = 웹뷰 페이지 로드 KPI — Parallel이 선행됐으니 이제 정확; **로드맵 미등록 신규 항목**) → 이후 **(3) B7-C 연결-단계 분해**(DNS/TCP/TLS/TTFB). 또는 B6(status-class+per-window RPS criteria, spec/plan 완료·구현 대기). 다음 단계 = `superpowers:brainstorming`으로 설계 의도 탐색.
...
### A7. Run 스케줄러 (예약/반복 실행) — **백엔드(34a+34b) ✅ 완료, 34c(UI) 잔여 (ADR-0034)**
...
- **연기**: 34c(UI) + 알림(이메일/슬랙/웹훅, 이음새=`schedule_events`)·per-schedule TZ(+DST 정책)·catch-up 모드·`runs.schedule_id` 역링크·이벤트 보존정책·프리셋에서 시드·멀티 컨트롤러(leader election).
...
- **run 목록 pass/fail 배지** (fast-follow): 목록엔 메트릭이 없어 영속화(`verdict_json` 컬럼 + 완료 시 평가, migration) 또는 목록용 경량 요약 캐시 필요. v1은 리포트 페이지만. (연기)
- **step-level criteria**: 특정 스텝 p95 등 — step_id 셀렉터 + loop/if 중첩 step_id 매칭. (연기)
- **일반 연산자 모델** (`{metric, op, threshold}` 자유 조합): 출력 shape는 이미 일반형이라 입력만 마이그레이션. (A4b run 비교·A4c 요약은 A4 영역의 별도 슬라이스.) (연기)
