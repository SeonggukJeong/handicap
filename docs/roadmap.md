...
- **A4c (actionable 리포트 요약) 🚧 spec+plan 완료, subagent-driven 구현 대기 (2026-06-03)**: 종료 run 리포트 최상단에 결정론적 인사이트 패널. 백엔드 `build_report`가 신규 순수 `insights.rs::derive_insights` → `ReportJson.insights: Vec<Insight>`(구조화 emit, UI 한국어 렌더). v1 6종: `slowest_step`·`error_hotspot`·`no_request_step`(시나리오 walk, 무조건-도달 스텝만, if 분기 제외)·`slo_failure`/`slo_pass`·`status_class`(4xx/5xx, status-0 분모 제외)·`status_temporal`(후반 5xx). 신호 있을 때만 emit(패딩 없음, run_health 의도적 미채택), `(severity_rank, table_row)` stable-sort. 단일-run XLSX **Insights 시트** 추가, CSV/compare/엔진/워커/proto/마이그레이션 무변경. **`#[serde(default)]`**(골든 fixture 호환) + strict `ReportSchema`에 키 추가. spec `docs/superpowers/specs/2026-06-03-a4c-actionable-report-summary-design.md`, plan `docs/superpowers/plans/2026-06-03-a4c-actionable-report-summary.md`(10 task). 둘 다 spec-plan-reviewer APPROVE-WITH-FIXES 반영 완료. **다음**: `EnterWorktree`(baseRef:head) → `cd ui && pnpm install` + `cargo build` baseline → Task 1부터 subagent-driven. **연기**: baseline 회귀(A4b 연계)·status-class verdict 기준(A4a')·CSV/compare 인사이트 → §B7.
