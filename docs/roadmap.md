...
- **연기: 부하모델 모드 선택기 (UX, 별도 슬라이스)** — RunDialog 상단에 "고정 VU(closed) / 고정 레이트 / 레이트 곡선" 라디오를 둬 *관련 필드만* 노출 → 영역 D의 상호배타 400들(target_rps↔ramp_up↔stages↔run-level think_time↔vus)을 **표현 불가능**하게 만듦(검증을 UI 구조로 흡수). S-D보다 넓은 RunDialog 리팩터라 분리. S-D(stages) 머지 후 착수 후보. (사용자 요청으로 기록 — 2026-06-04 S-D brainstorm.)
