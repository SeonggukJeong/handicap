...
- **R6 — 저장 용량 가드(신규)**: 시나리오 생성/수정 API 호출 직전, 직렬화된 요청 body가 **2MiB(2,097,152B) 이상**이면 fetch 없이 한국어 에러를 throw — 기존 에러 Callout 경로로 표출, 현재 크기(MB) 포함. 서버가 413을 돌려주는 경우(클라 체크 우회·한도 차이)도 같은 문구로 매핑. 문구는 `ko.ts` 카탈로그 경유(ADR-0035).
...
**yamlDoc** (`ui/src/scenario/yamlDoc.ts`): `default_think_time`의 set/delete 이디엄(yamlDoc.ts:146–153)과 동일 — `setNotes` edit: 값 있으면 `doc.setIn(["notes"], …)`(멀티라인은 yaml 라이브러리가 block scalar로 직렬화), undefined면 `doc.deleteIn(["notes"])`.

**store** (`ui/src/scenario/store.ts`): `setNotes(value: string | undefined)` 액션 — `setDefaultThinkTime`(store.ts:55) 이디엄. dirty 전파는 기존 경로 그대로.
...
- 시각: `Callout` `info` 변형(`ui/src/components/ui/Callout.tsx` — accent-200/50/800) 재사용. 제목 "📝 공유 메모"(`ko.ts` 신설 키). 접힘/펼침·편집 버튼은 aria-label 분리(WCAG Label-in-Name).
- 접힘 localStorage: `handicap:scenario-notes-collapsed:v1` 단일 키에 `{ [scenarioId]: true }` 맵(fail-soft try/catch — editorPrefs 이디엄). 메모가 삭제된 시나리오 키는 다음 로드 시 정리. scenarioId는 EditorShell에 없으므로 컴포넌트가 react-router `useParams`로 직접 취득(신규 페이지는 id 부재 → 세션 상태 폴백, prop 배선 0).
...
**저장 용량 가드** (`ui/src/api/client.ts`): 시나리오 create/update 함수에서 `new Blob([JSON.stringify(body)]).size >= 2 * 1024 * 1024`면 fetch 전에 `Error(ko.…(sizeMb))` throw → 페이지의 기존 `update.error`/`create.error` Callout이 표출. 413 응답도 동일 문구 매핑. **사실 근거**: 컨트롤러 body limit 상향(256MiB, app.rs:35)은 데이터셋 라우트에만 적용 — 시나리오 라우트는 axum 기본 2MiB. SQLite TEXT 자체 상한은 ~1GB(SQLITE_MAX_LENGTH 기본)라 실질 상한은 2MiB 쪽.
...
- 클라 가드 우회(curl 직접 POST) 대용량 → 서버 413(기존 동작) — UI 경유 시 R6 매핑 문구.
...
- **UI(RTL/vitest)**: 4상태 전이(표출→접기→펼치기→편집→완료/취소), 공백-only 완료 시 store에서 키 삭제, YAML 모달 왕복(R8), 사이즈 가드(2MiB 초과 body에서 fetch 미발생+문구), Zod 모델 optional 라운드트립. **회귀 가드 표방 테스트는 고의 회귀→RED→원복→GREEN 실증**([[plan-mandated-vacuous-tests]]).
