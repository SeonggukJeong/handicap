...
S-C의 격리 함수 `run_scenario_open_loop`에 **레이트 곡선만 주입**한다. 슬롯풀·drop·`dropped` 카운터·`exhausted`·cancel·플러셔·`run_arrival`은 100% 무변경 — 곡선은 "언제 arrival을 발사하나"만 바꾸고 슬롯/집계/메트릭은 안 바꾼다.
...
- **interval 계산**: rate > 0이면 `interval = 1 / rate_at(elapsed)`, `next += interval`. 저레이트면 interval이 자연히 커짐(정상, ramp-down 꼬리) — **interval에 상한을 두지 않는다**(상한은 레이트를 왜곡: 0.5 RPS=2s 간격을 1s로 자르면 1 RPS가 됨). 긴 sleep의 responsiveness는 기존 S-C 코드의 `tokio::select! { sleep, cancel }` + 루프 top의 deadline 체크가 이미 보장(긴 sleep도 cancel로 깨지고 deadline에 막힘).
- **rate ≈ 0 가드**: `rate_at`이 0(또는 epsilon 미만)이면 **arrival을 발사하지 않는다**(spawn도 drop도 없음 — drop은 rate>0인데 슬롯이 없을 때만). 고정 poll-step(예 100ms)만큼 sleep(select! cancel) 후 재평가. 자연 발생 지점은 ① 마지막 target=0 stage 끝(= deadline, 루프가 break) ② 중간 `{0, d}` hold(드물지만 표현 가능 — 그 구간 arrival 0, 정확). rate가 다시 >0으로 오르면 `interval = 1/rate` 재개.
- **`deadline = started_at + sum(stage durations)`**.
...
| 레이어 | 변경 |
|---|---|
| **엔진** `scenario.rs`/`runner.rs` | `Stage { target: u32, duration_seconds: u32 }`, `RunPlan.stages: Option<Vec<Stage>>`, `rate_at` + 곡선 스케줄러 |
| **proto** `handicap.proto` | `message Stage { uint32 target = 1; uint32 duration_seconds = 2; }` + `repeated Stage stages = 10;` (Profile). repeated → 비어있음 = absent |
| **store** `store/runs.rs::Profile` | `stages: Option<Vec<Stage>>` `#[serde(default, skip_serializing_if = "Option::is_none")]` |
| **검증** `api/runs.rs::validate_run_config` | §3.3 상호배타 + §3.4 bounds. open-loop 트리거를 `target_rps.is_some() || stages 비어있지 않음`으로 확장 |
| **worker** `main.rs` | open-loop 디스패치 조건 확장, `RunPlan.stages` 빌드, `duration = stages 있으면 sum / 없으면 기존`. proto `repeated` → `Vec` → 비었으면 `None` 매핑 |
| **메트릭/리포트** | **무변경** — `MetricBatch`/`Aggregator`/`build_report` 그대로. 초당 RPS 윈도가 곡선을 이미 드러냄(새 집계 0) |
| **UI** | §6 |

prost exhaustive 함정: `repeated Stage stages = 10;` 추가 시 `Profile { … }` struct literal 사이트는 `stages: vec![]`만 더하면 됨(repeated는 default empty). `Stage`는 신규 메시지라 literal 사이트 없음. (`MetricBatch` 무변경이라 그쪽 grep 불요.)
