...
- **측정 토글(기존 744-751, 고급 그룹 내부)** → 상세 독립 섹션으로 승격(R9) — 가치 라벨 + HelpTip. 고급 그룹엔 SLO/스텝SLO/페이싱/http_timeout/loop_cap만 남김. **측정이 collapse 밖으로 나가지만 `advancedActiveCount`(319-320)는 그대로 둔다**(measure 포함 — `detailedAppliedCount`가 이걸로 measure를 계수). 대신 **고급 collapse 접힘 힌트(631-633)만 새 `advancedSetHint = sloActiveCount + (loadModel==="closed"?pacingActiveCount:0)`**(measure 제외)로 바꿔 빈 collapse가 "1개"로 보이는 off-by-one 차단(리뷰 A). measure는 `detailedAppliedCount`에서 `advancedActiveCount` 경유로 **1회만** 계수(별도 항 추가 금지=이중계수 방지).
