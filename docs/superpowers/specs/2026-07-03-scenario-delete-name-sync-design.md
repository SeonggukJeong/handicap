...
- **접근 비교(요약)**: cascade = 명시적 트랜잭션 삭제(store 함수) — **채택**. 마이그레이션으로 `ON DELETE CASCADE` 추가는 **기각**(SQLite는 기존 테이블 FK 변경 불가 — 테이블 재생성 필요, 위험 대비 이득 없음; migration 0005 주석도 엔드포인트 생길 때 처리를 전제). 삭제 진입점은 목록 행만(에디터 페이지 중복 진입점 기각 — 범위). 이름 편집은 h2 옆 연필→input 전환(항상-input 헤딩은 `<h2>` 시맨틱 상실로 기각).
...
| R2 | MUST **hard 가드**: 이 시나리오의 `pending`/`running` run이 하나라도 있으면 `409 Conflict`(문자열 에러, 한국어) — **`force=true`여도 거부**(실행 중 부하의 발밑 삭제 방지; 데이터셋 hard 가드와 동일 정신, 단 판정은 `runs.scenario_id` 직접 SQL이라 profile_json 파싱 불요). | API 테스트: running run 심고 force=true 포함 409 단언. | |
...
| R7 | MUST 에디터 헤더 h2·브레드크럼이 `model.name` **라이브 렌더**(`liveName = editorModel?.name ?? data.name` — YAML 편집 즉시 반영, 깨진-YAML 윈도(`model===null`)는 서버명 폴백). | RTL: YAML/스토어 name 변경 → h2·브레드크럼 즉시 갱신; model=null 폴백. | |
| R8 | MUST h2 옆 **연필 버튼 → 인라인 `<input>` 편집**: draft 시드=liveName, **Enter/blur 커밋**(trim 후 **빈 문자열이면 revert** — `ScenarioModel.name: min(1)`이라 빈 커밋은 doc/model 갈라짐[URL-min(1) 함정 동류]), 유효하면 `setName(trimmed)` 재사용 → YAML doc 반영 → 기존 dirty·저장 흐름 합류; **Escape 취소**(revert). `model===null`이면 연필 disabled + title 안내. | RTL: 커밋→h2 갱신+dirty 활성 / 빈이름 revert / Escape revert / model=null disabled. | 신규 상태·스키마 0 |
| R9 | MUST **false-dirty 제거**: `ScenarioEditPage`가 data 도착 시(시나리오 **id당 1회**) `loadFromString(data.yaml)` → 그 시점 canonical store `yamlText`를 로컬 `yamlText`/`originalYaml` 양쪽에 시드 → **시드 완료 후에만 `EditorShell` 마운트**(게이트). `baselineSeededRef` 제거, `handleEditorChange`는 단순 `setYamlText`. 재시드는 **id 변경 시만**(저장 후 version bump는 기존 save 핸들러의 `originalYaml=next.yaml` 유지로 충분). | RTL(회귀 — 현재 버그라 RED부터): 로드 직후 저장 버튼 **disabled**; 편집 후 enabled; 저장 후 다시 disabled. | |
| R10 | MUST 신규 사용자 노출 문구(confirm·배너·aria-label·연필 title 포함) 전부 `ko.ts` 카탈로그 경유(ADR-0035) — 카운트 문구는 함수 키. | grep: 신규/변경 파일 하드코딩 한국어/영어 문구 0(`'"[^"]*[가-힣]'` 스윕). | |
| R11 | MUST (불변식) 엔진·worker·proto·**migration 0건**(테이블 변경 없음); `schemas.ts`·`scenario/model.ts`·`store.ts`·`yamlDoc.ts` **무변경**(이름 편집은 기존 `setName` 재사용); 시나리오 YAML 와이어 포맷 byte-identical. backend diff = `app.rs` 라우트 + `api/scenarios.rs` 핸들러 + `store/` 헬퍼(`scenarios.rs` cascade + `runs.rs`/`presets.rs`/`schedules.rs`의 active-run 판정·카운트)뿐. | 머지 diff 경로 검사 + 기존 테스트 전부 green. | |
...
5. **스케줄러 레이스는 FK가 fail-fast로 막는다**: 스케줄러 tick이 삭제 직전 로드한 스케줄로 run을 spawn하려 하면 `runs` INSERT가 FK로 실패 → 기존 tick 에러 경로에 로깅되고 다음 tick엔 스케줄 행이 없다(스케줄러는 매 tick DB 조회 — in-memory 잔존 없음). `POST /api/runs` 동시 요청도 동일(윈도 극소·FK fail-fast — 수용). 별도 락 불요.
...
`DeleteQuery{force: bool}`(datasets 미러, `#[serde(default)]`) + `delete(State, Path, Query)` 핸들러: 존재 확인(404) → hard 가드(`store::runs::scenario_has_active_runs` — `SELECT EXISTS(… runs WHERE scenario_id=? AND status IN ('pending','running'))`) → soft 가드(force=false && 카운트 합>0 → `ApiError::ConflictJson`) → `store::scenarios::delete_cascade` → 204. 에러 문구는 datasets 관용구대로 핸들러 내 한국어 리터럴(백엔드는 ko.ts 범위 밖).

### 4.3 `crates/controller/src/store/scenarios.rs`(+ runs 카운트 헬퍼) — 충족 R: R2, R3, R4
`delete_cascade(db, id)` — `db.begin()` 트랜잭션으로 §R4 순서 삭제. 카운트 쿼리(`runs`/`run_presets`/`schedules` per scenario_id)는 소속 store 모듈에 배치.
...
삭제 confirm 2종(함수 키 — 이름/카운트 보간)·hard 409 배너·삭제 버튼 라벨·연필 aria-label/title·이름 input aria-label 등 신규 키.
...
| 5 | 스케줄러 tick이 삭제 직전 스케줄 로드 | run INSERT FK 실패 → tick 에러 로깅, 다음 tick 소멸(§3-5, 수용) |
