...
- **날짜**: 2026-07-03 (개정 r3 — round 1: 가드 in-tx 이동·liveName seeded 게이트·loadedVersion id-키드 핀·0005 주석 갱신+ADR, 스코프 분리 기각[사용자 결정 번들·plan이 삭제/에디터 task 그룹 독립 green 담보]. round 2: r2가 만든 edge#1↔R2 모순 해소 — 핸들러 advisory hard fast-fail 복원[권위는 in-tx 유지], spurious 500 폭 주석)
...
| R2 | MUST **hard 가드**: 이 시나리오의 `pending`/`running` run이 하나라도 있으면 `409 Conflict`(문자열 에러, 한국어) — **`force=true`여도 거부**(실행 중 부하의 발밑 삭제 방지; 데이터셋 hard 가드와 동일 정신, 단 판정은 `runs.scenario_id` 직접 SQL이라 profile_json 파싱 불요). **권위 판정은 cascade 트랜잭션 *안*의 `SELECT EXISTS`**(§3-5 레이스 봉쇄) — `delete_cascade`가 활성 run 발견 시 롤백 + 구분 가능한 에러 반환, 핸들러가 409로 매핑. 추가로 **핸들러가 advisory hard fast-fail**: 카운트 조회(R3와 같은 쿼리)에 활성 run 수를 포함해 활성>0이면 force 값 무관 **soft 가드보다 먼저** hard 409 문자열 반환 — soft confirm("함께 삭제됩니다")이 삭제 불가능한 시나리오에 뜨는 기만 UX 방지(advisory는 UX용, 권위는 in-tx — 이중이 아니라 계층). | API 테스트: running run 심고 **force=true·false 둘 다** hard 409 문자열 shape 단언(soft JSON 아님) + store 테스트: 활성 run 존재 시 `delete_cascade`가 에러 반환·전 테이블 무변이. | |
...
5. **삭제 레이스는 "in-tx 가드 + FK + WAL 단일-writer"의 3겹으로 봉쇄** (reviewer round 1이 가드-밖 인터리빙 적발): hard 가드를 핸들러(tx 밖)에 두면 "가드 SELECT 통과 → 새 run INSERT 커밋 → cascade가 그 pending run 행을 삭제 → 워커는 run 행 없이 부하 발사(메트릭 ingest FK 실패는 warn-only)"라는 **silent 좀비-부하 윈도**가 생긴다. 그래서 권위 가드를 `delete_cascade` 트랜잭션 **안**으로(R2/R4): ① tx 시작 후 삽입된 run은 in-tx EXISTS가 본다(→409) ② EXISTS와 DELETE 사이에 끼어드는 동시 쓰기는 SQLite WAL에서 busy/snapshot 에러로 tx 실패(→500, fail-loud — deferred BEGIN 특성; 트리거는 racing run-INSERT만이 아니라 그 마이크로-윈도의 **모든** 동시 커밋[무관 run의 메트릭 ingest 등]이라 드물게 spurious 500이 날 수 있으나 재시도 가능·무손상이라 수용, 원하면 plan에서 `BEGIN IMMEDIATE` 채택 가능) ③ cascade 커밋 *후*의 스케줄러 tick·`POST /api/runs`는 `scenarios` 행이 없어 `runs` INSERT가 FK로 실패 → 기존 tick 에러 경로 로깅(spawn 실패 → `errored`+`mark_outcome`), 다음 tick엔 스케줄 행도 소멸. 별도 락 불요.
...
`DeleteQuery{force: bool}`(datasets 미러, `#[serde(default)]`) + `delete(State, Path, Query)` 핸들러: 존재 확인(404) → 카운트 조회(활성 run 수 포함) → **활성>0 → hard 409 문자열(advisory fast-fail, force 무관)** → soft 가드(force=false && 참조 합>0 → `ApiError::ConflictJson`) → `store::scenarios::delete_cascade` 호출, 활성-run 구분 에러면 동일 hard 409로 매핑 → 204. 권위 판정은 in-tx(§4.3), 핸들러 fast-fail은 advisory UX. 에러 문구는 datasets 관용구대로 핸들러 내 한국어 리터럴(백엔드는 ko.ts 범위 밖).
