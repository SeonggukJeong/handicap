...
- **US1**: QA가 **시나리오를 편집하는 동안** 이 시나리오가 믿을 만한 시험인지 판단하려 한다 — 성공하면 에디터에서 등급과 통과 카운트(`보완 필요 · 점검 3개 중 1개 통과`)를 보고, 결함을 고치는 즉시 등급이 올라가는 것을 본다.
...
| D9 | 색 | **양호 = 중립 회색**. 보완 필요 = amber, 취약 = red | 거짓 초록과 싸우는 기능이 새 초록을 만들면 자기모순 |
...
- 문구: `◈ 신뢰도 · 양호` / `◈ 신뢰도 · 보완 필요 2` / `◈ 신뢰도 · 취약 1`. 숫자 = `failed`(최대 3).
...
- 색: 양호 = 기존 칩과 같은 중립(D9), 보완 필요 = amber, 취약 = red 계열 텍스트.
- **`aria-label`/`title`**: 모호성 해소(칩 숫자 vs 모달 카운트) + **US5 경계 문장**(D10). **문구 정본은 `docs/dev/scenario-preflight-copy.md`의 `## 칩 접근명 조립 규칙`이다 — 이 절이 처음 적었던 `시나리오 신뢰도: …` 형태는 폐기됐다.** 구현 리뷰에서 그 형태가 **WCAG 2.5.3 Label-in-Name 위반**(접근명이 가시 라벨을 포함하지 않음 — 구분자가 `·`인데 접근명은 `:`, `(미확인)`·숫자가 접근명에 부재)임이 드러나, 접근명을 `가시 라벨 + " " + 꼬리`로 **조립**하도록 바꿨다(가시 텍스트를 한 번만 만들어 JSX와 접근명이 함께 쓰므로 드리프트가 구조적으로 불가). 근거·3상태 표는 카피 정본의 `## 개정 이력`. **접근명을 다시 `시나리오 신뢰도: …`로 "복원"하지 말 것 — 그게 Level A 위반이다.**
...
- 그 외: `◈ 시나리오 신뢰도: 보완 필요 (2건)` + `에디터에서 보기` 링크(`/scenarios/{scenarioId}`). `2건` = `failed`.
...
**등급 어휘**(1차의 `해석 가능/제한적 해석/해석 주의`와 겹치지 않는다): `good`=`양호`, `caution`=`보완 필요`, `weak`=`취약`.
...
6. **점검 억제(opt-out) 없음**(D18): 워밍업/teardown처럼 정당하게 assert 없는 스텝이 하나만 있어도 A는 영구 실패하고 등급이 `보완 필요`로 고정된다. 그럼에도 만들지 않는다 — "검증 없는 스텝"을 침묵시키는 장치는 이 에픽이 싸우는 대상을 **정확히** 숨긴다. 실사용에서 이 고정이 실제 마찰이 되면 그때 근거를 갖고 재검토한다.
