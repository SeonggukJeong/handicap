...
목업의 시각 요소 중 normative인 것: 와이드 행 부가 칩(검증 N·추출·think), 칩 스트립 1줄(흐름 라벨+`→` 구분), 점프된 행 하이라이트, 편집 모달 안 접이식 섹션 재사용. 나머지(색·간격)는 기존 디자인 토큰/아웃라인 스타일을 따른다.
...
| R2 | MUST 접힌 disclosure에 내용이 있으면 힌트 배지 표시 — 헤더 "N개"(활성+비활성 합), 검증 "N개", 추출 "N개", 바디는 kind 라벨(JSON/폼/텍스트, none이면 배지 없음), 타이밍 "설정됨"(타임아웃 또는 think 존재 시). | RTL: fixture 스텝으로 각 배지 텍스트 정확-매치 단언 | |
...
| R7 | MUST 와이드 ON 시 목록 상단에 흐름 칩 스트립(`TestFlowChips` 재사용, `trace=null`)을 렌더하고, 칩 클릭은 선택+해당 행 scrollIntoView+선택 하이라이트만(편집 모달 미오픈). | RTL: 칩 스트립 존재(`role="group"` within-스코프)+칩 클릭 후 모달 부재; Playwright: 칩 클릭 시 목록 `scrollTop` 변화 실측 | |
| R8 | MUST 와이드 ON에서 아웃라인 행 활성화(클릭/키보드 Enter — 기존 행 활성화 경로와 동일)는 선택+편집 모달(기존 `Modal`+`Inspector` 재사용, R1–R3 동작 그대로)을 열고, ✕/ESC/backdrop 닫기 시 선택은 유지, 모달 내 스텝 삭제 시 모달이 닫힌다(선택 clear 경유). | RTL: 행 클릭→모달 내 인스펙터 렌더·닫기 후 선택 유지·삭제 시 모달 부재 | |
...
| R12 | MUST 4곳(http/loop/if/parallel) 이름 입력을 draft+하이브리드 커밋으로 — 비어있지 않으면 onChange 즉시 커밋(아웃라인·칩 라이브 갱신 유지), 빈 값은 미커밋(draft 유지), blur 시 draft가 trim-빈이면 "Untitled" 커밋; 타이핑 중 "Untitled" 스냅 없음; draft는 step.id 전환 시 재시드. | RTL: 이름 전체 삭제 → 입력값 ""·store는 직전 이름 유지 → blur → 둘 다 "Untitled"; 부분 수정 타이핑 중 스냅 없음 | |
...
1. **와이드 = 전폭 세로 리스트(§0 확정)**: "넓게 보기"의 목적이 *스텝 내용을 더 보는 것*(잘리던 URL·부가 정보)이므로 리스트를 넓히는 A가 정답. 가로 카드(B)는 구 캔버스 감각이지만 카드 폭이 좁아 URL이 다시 잘리고, 드래그(세로 리스트 전제·`nearestByHeader` y-기반)를 재작업해야 하며 ADR-0044가 캔버스를 걷어낸 이유로 회귀한다. B의 유일 장점(가로 조망)은 기존 `TestFlowChips` 1줄 재사용으로 흡수(v2 확정) — 새 UI 발명 없음.
2. **칩=점프, 행=모달**: 모달 오픈을 선택 상태의 부수효과로 만들면 칩 클릭(선택 변경)도 모달을 열어버린다 — 모달은 행 활성화 핸들러가 명시적으로 연다(R7/R8 분리). `TestFlowChips`는 `steps`/`selectedStepId`/`onSelect`만 받는 프레젠테이셔널이라 trace 없이 재마운트 가능(test-run 섹션의 기존 마운트와 독립).
...
6. **기존 인스펙터 테스트 대량 갱신은 계획된 비용**: 접힘 기본(R1)은 헤더/바디/검증/추출 입력을 직접 집는 기존 RTL을 깨뜨린다 — RunDialog SLO 접이식 전환(A4a follow-up) 때와 같은 클래스로, 각 테스트 선두에 disclosure 펼침을 추가한다(동작 변경이 아니라 접근 경로 변경).
...
- 공용 `InspectorSection`(제목·`aria-expanded` 토글 버튼·힌트 배지·`{open && children}`) 도입 — RunDialog SLO disclosure 이디엄(`<button aria-expanded>` + caret `▸/▾`).
...
- ON: 그리드를 `[변수?][1fr 아웃라인]`으로 전환(인스펙터 열 미렌더), 아웃라인 컨테이너에 뷰포트-기준 최대높이(`calc(100vh - 오프셋)`, 오프셋은 구현에서 실측) + `overflow-y-auto`; 칩 스트립(`TestFlowChips steps={model.steps} trace={null} selectedStepId onSelect=점프`)은 스크롤 영역 **밖** 상단.
- 편집 모달: `open={wideOpen && detailOpen && selectedStepId!=null}`로 `<Modal title=ko…><Inspector/></Modal>` — `removeStep`이 선택을 먼저 clear하므로 삭제 시 자동 닫힘(R8). 행 활성화 → `detailOpen=true`, 칩 클릭 → 선택+scrollIntoView만.
...
- 행에 `data-step-id`(scrollIntoView 타깃) 부여, 와이드 시 http 행 부가 칩(검증 N·추출 N(title=var 나열)·think min–max) 렌더. 행 클릭/Enter는 기존 select에 더해 `onActivateStep` 호출(와이드에서만 전달됨). 드래그 경로(`reorder.ts`·`dropRules.ts`) 무변경.
...
- 신규 키: 와이드 토글 라벨/aria, 섹션 제목 5종(기존 키 재사용 가능하면 재사용: `headersLabel` 등), 힌트 배지 포맷(`N개`·"설정됨"·바디 kind 라벨), 편집 모달 제목, 흐름 스트립 라벨(기존 `TestFlowChips` 키 재사용).
...
|---|---|---|
| R1·R2 | `Inspector.sections.test.tsx`(신규): 기본 접힘·배지 정확-매치·펼침 시 편집기 렌더 + 기존 Inspector 테스트에 펼침 선행 추가 | |
...
| R8 | RTL 행 클릭→모달·닫기 후 선택 유지·삭제 시 닫힘 | |
...
UI-only라 cargo 게이트는 매 커밋 skip(fast-path). 각 task 독립 green 커밋:
