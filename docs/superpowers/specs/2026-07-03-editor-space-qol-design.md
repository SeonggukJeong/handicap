...
| R8 | MUST 와이드 ON에서 아웃라인 행 활성화(클릭/키보드 Enter — 기존 행 활성화 경로와 동일)는 선택+편집 모달(기존 `Modal`+`Inspector` 재사용)을 열고, ✕/ESC/backdrop 닫기 시 선택은 유지하며, `detailOpen`은 ①모달 onClose ②`selectedStepId===null` 전이(모달 내 스텝 삭제 포함) ③와이드 OFF 세 지점에서 리셋되어 **삭제 후 칩 점프·와이드 재토글이 모달을 재오픈하지 않는다**. | RTL: 행 클릭→모달 내 인스펙터 렌더·닫기 후 선택 유지·삭제 시 모달 닫힘·**삭제 후 칩 클릭 시 모달 미재오픈** | |
...
- **알려진 한계(수용·§7 연기)**: 모달 내 팝오버(`VarCheatSheet`·JSON-cast `HelpTip`)가 열린 채 ESC를 누르면 팝오버가 아니라 모달이 닫힌다 — `Modal.tsx` capture-phase ESC `stopPropagation` vs HelpTip bubble-phase ESC(ui/CLAUDE.md 문서화 함정). 편집 손실 없음(커밋은 즉시/blur·선택 유지·재오픈 가능)이라 수용; "Inspector 동작 그대로"의 명시적 예외.
