...
- **프리필 중복 방지(dedupe)**: 서로 다른 환경이 같은 이름(전형: 둘 다 `BASE_URL`)을 쓰면 2-host HAR에서 두 행이 같은 이름으로 프리필돼, 미게이트 [복사]/[편집기로 보내기] 경로로 **사용자 행동 없이** 두 host가 한 `${BASE_URL}`로 붕괴한다(silent-config 클래스). 방지 알고리즘:
  1. **pre-pass**: `hostsOrdered`에 현재 존재하는 host의 `overrides` 값을 전부 `overrideNames` 집합에 시드 — override 칸은 절대 재작성하지 않지만 **프리필 후보를 차단한다**(시드를 현재 host로 한정하는 이유: overrides는 파일 재선택 전까지 잔존하므로 제외된 host의 stale 항목이 이름을 예약하면 안 됨). 이게 없으면 "뒤 행에 override + 앞 행에 매치" 순서에서 늦은 프리필이 override와 같은 이름을 배정해 동일한 silent-collapse가 남는다.
  2. **forward pass**: `hostsOrdered` 순서로 순회 — override 있는 host는 그 값 그대로(재작성 없음). 나머지 배정은 **두 집합을 구분**한다(`overrideNames`=pre-pass 시드 · `usedByPrefill`=이 pass가 배정한 이름): **후보**는 `overrideNames ∪ usedByPrefill` 어디에도 없어야 자격 / **`defaults[h]`**는 `usedByPrefill`에만 없으면 배정 — override와의 충돌은 일부러 막지 않는다(사용자가 타이핑으로 만든 기존-가시 충돌이라 `validateEnv` dup 빨간 문구가 잡고, 막으면 매치 0건 경로가 개명돼 R8 byte-identical이 깨진다) / **`BASE_URL_{k}`**는 **k=2부터 증가**하며 `overrideNames ∪ usedByPrefill`에 없는 첫 이름(`defaultHostVars`는 `BASE_URL_1`을 절대 내지 않음). **프리필이 배정한 모든 이름은 즉시 `usedByPrefill`에 추가** — 이 불변식이 뒤 host의 프리필-유래 무충돌을 보장한다. 결정적이며 프리필이 만드는 충돌은 0.
...
- **비동기 도착 중 YAML 플립**: 사용자가 매치 도착 전에 hostVars를 켜면 미리보기 토큰이 `BASE_URL`→매치명으로 한 번 바뀔 수 있다 — 수용(HAR 파싱 직후 fetch 시작이라 실사용상 창이 짧고, override는 불변). **매치 없는 행도 dedupe 폴백으로 이름이 한 번 바뀔 수 있다**(예: 앞 행의 매치명이 이 행의 기본명 `BASE_URL_2`를 점유 → 이 행은 `BASE_URL_3`) — 결정적·미편집 행 한정이라 같은 수용 범주. 라이브 검증 시 "매치 없는 행이 혼자 바뀜"을 결함으로 오판하지 말 것. 스펙 한계로 기록.
