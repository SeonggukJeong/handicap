...
- ✅ **라우팅 패턴.** `app.rs` 가 라우트를 빌드 후 `.nest("/api", api)`(`:71`). presets `/scenarios/{id}/presets`·`/presets/{id}`(`:60-69`), datasets `/datasets`·`/datasets/{id}`(`:46-59`). → `/environments`·`/environments/{id}` 추가. `api/mod.rs` import 목록(`datasets as datasets_api, presets as presets_api, …`)에 `environments as environments_api` 추가.
...
- **`UNIQUE(name)`** — 환경은 top-level 이라 이름 전역 유일. dropdown 이 이름으로 구분, 같은 이름 저장 = 덮어쓰기 충돌(409 → UI 가 안내). `CREATE UNIQUE INDEX IF NOT EXISTS` 로 멱등.
...
- 라우팅: `app.rs` 에 `/environments`(POST/GET), `/environments/{id}`(GET/PUT/DELETE) 추가. `api/mod.rs` 에 `pub mod environments;` + import alias.
...
- `EnvironmentsPage.tsx`: 목록 + 생성/편집(name + `{key,value}` 행 추가/편집/삭제) + 삭제. `DatasetsPage.tsx` 미러. 네비게이션 항목 추가.

**RunDialog 환경 선택 + 오버레이**:

- Env 섹션 상단에 **환경 dropdown** 추가. 옵션 = "(없음)" + 환경 이름들. 기본 = 없음.
- **레이어드 상태**: `selectedEnvId`(신규) + 기존 `envEntries`(이제 **override 레이어**). 선택 환경의 vars 는 **base** 로 렌더(예: "from `staging`" 라벨, 읽기 표시), `envEntries` 가 그 위 override.
- **병합·제출**: 제출 시 `resolveEnv(selectedEnv?.vars ?? {}, envEntries)` = `{ ...base, ...overrides }`(override 승) 결과를 기존 평탄 `env` 맵으로 제출(스냅샷).
- **환경 전환**: base 만 교체, `envEntries`(override)는 유지.
- **prefill 상호작용**: 프리셋/retry 로 열 때(`initial.env`)는 `envEntries` 를 그 해석값으로 채우고 환경 = 없음(스냅샷이 이미 해석값 보유 — 충돌 없음). 영역 A1 prefill 경로와 그대로 양립.
...
- **var 키**: 비어있지 않은 식별자, 공백·`}`·`:` 금지 — 그래야 엔진 `${NAME}` 으로 실제 해석된다(`:-` 는 엔진 기본값 구분자라 키에 들어가면 영영 미해석). 예약 시스템 변수(`vu_id`/`iter_id`/`loop_index`)와 같은 이름은 경고(엔진이 시스템 값으로 우선 해석 → env 값 무시됨, `template.rs:115-123`). v1 은 거부까지는 안 하고 검증 경고만(authoring 친화).
- **var 값**: 문자열만(이미 A1 `CreateRunRequest.env: HashMap<String,String>` + Zod `z.record(z.string())` 가 경계에서 강제).
...
- 게이트: `pnpm build`(`tsc -b`) — Zod `z.record` 출력 타입·중첩 default 누출 확인(ui/CLAUDE.md).
