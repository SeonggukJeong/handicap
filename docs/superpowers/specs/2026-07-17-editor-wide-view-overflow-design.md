...
3. 재측정 메커니즘: mount 시 1회 측정(`useLayoutEffect`, deps `[expandable]`) + 이후 재측정은 **`ResizeObserver` 전담**(콘텐츠 증감·폭 변경 공통) — overflow 불리언이 바뀌는 모든 전이는 캡 경계(96px)를 지나며 wrap 박스 높이가 실제로 변하므로 RO 발화가 보장된다(이미 캡에 눌린 상태의 내부 scrollHeight 증감은 박스 불변=무발화지만 상태도 불변이라 무해). deps에 `steps`/`trace`를 넣는 설계는 effect 본문이 이를 미참조라 `react-hooks/exhaustive-deps` '불필요 dep' 경고(`--max-warnings=0` 실패)로 기각(plan 단계 발견). RO 콜백은 읽기+`setState`만(스타일 미변경이라 `AutoGrowTextarea`류 되먹임 루프 없음 — same-value setState는 React가 bail). `disconnect` cleanup. **측정 effect·RO 생성은 `expandable`로 게이트** — EditorShell 인스턴스(미전달)는 `overflowing`을 소비하지 않으므로 관측 자체를 만들지 않는다(불필요 RO 방지).
...
- **jsdom은 scrollHeight/clientHeight 항상 0 → overflow mock 메커니즘은 다음 중 하나로 핀** (둘은 등가가 아님 — 측정 `useLayoutEffect`가 mount 중 동기 실행되므로 render *후* element-레벨 `defineProperty`만 하면 재측정 트리거가 없어 T3/T4가 vacuous): ⓐ **`vi.spyOn(Element.prototype, "scrollHeight"/"clientHeight", "get")`을 `render()` 호출 *전*에 설치**(권장 — mount-시 측정이 mock 값을 봄; jsdom에서 이 getter들은 `Element.prototype`의 own property라 `HTMLElement.prototype` 대상 spyOn은 'property does not exist'로 throw — 2026-07-17 node 실측), 또는 ⓑ element `defineProperty` + RO mock 콜백 수동 발화로 재측정 유도(`AutoGrowTextarea.test` 선례, T6 경로와 겸용). teeth: mock 제거 시 토글 부재 FAIL 확인.
...
- RO-전담 재측정의 알려진 한계(plan 리뷰 LOW, 수용): ① steps 0→>0로 wrap div가 재마운트되면 새 div를 재관측하지 않아 `overflowing`이 stale 가능(expandable 인스턴스에서 스텝 전부 삭제 후 재추가하는 드문 경로 — 토글 가시성만 영향, 리마운트로 회복; 코드 comment로 표기) ② 전이 보장은 칩 행 높이가 캡 96px에 정확히 정렬되지 않는 현 스타일링에 의존 — 캡/칩 사이징 변경 시 재검증.
