...
3. **토큰 소실 진단 2분화 (리뷰 R1/M2 결정)**: 소스-제외 후 "소스에 산문으로 남았지만 목적지엔 없는" 토큰(정당한 backtick 해제 리라이팅 포함)이 기존 문구 `소실(목적지 어디에도 없음)`으로 찍히면 **거짓 진단**이다(텍스트는 소스에 실재). 토큰 루프에서 `t in cur_src`(raw text)를 추가 검사해 그 경우 별도 문구로 FAIL: `FAIL [토큰] 소스에 산문으로 잔존 — 목적지 미확인: <t>`. 여전히 blocking(게이트 약화 아님) — 운영자가 정확한 원인으로 allowlist/이동 판단을 하게 하는 것이 목적.
4. **`[move]` FAIL 문구 소스-중립화**: 기준 = **"root" 오지칭이 있는 문구만** 개명(리뷰 C-b — "verbatim 인용 여부"는 기준이 못 된다; 인용 문서는 §2.4 sweep이 따라간다): `base root에 anchor 없음` → `base 소스에 anchor 없음`, `root에 anchor 잔존(미제거)` → `소스에 anchor 잔존(미제거)`. 다른 FAIL 문구(manifest·R17·R18·토큰 기존 문구)는 "root" 오지칭이 없으므로 불변. 내부 변수 `base_root`/`cur_root` → `base_src`/`cur_src` **일괄 13곳**(C9 — plan이 grep으로 개수 재고정해 부분 개명 방지).

### 2.2 `scripts/doc-coverage-allowlist.txt`

- 아티팩트 5행(현행 16–20행, "ui-curation 토큰 아티팩트" 근거 행 전부) **삭제** — 2.1-1 이후 차분에서 전소멸(C2)이라 죽은 행이다.
- 실식별자 4행(12–15행)은 **유지** — 정확한 이유(리뷰 F5): 이 4행은 **root 소스 실행의 토큰 검사에서 지금도 유효한 억제자**다(17369d32 base 차분에 4건 전부 등장 — 삭제 시 그 실행에서 `FAIL [토큰]` 4건). 단 root 소스 실행 자체는 현행 61행 ui manifest 탓에 `[move]` FAIL 23건으로 어차피 green 불가 — 즉 "역사적 root 차분 재현 시 토큰 클래스만이라도 정직하게 유지"가 목적이다. 현행 기본 실행(ui 소스)의 차분엔 미등장(inert — C6).
...
1. **`docs/dev/root-doc-maintenance.md`** (편집 4건 — 리뷰 R2로 1건 추가):
...
```bash
grep -rn "base root에 anchor\|root에 anchor 잔존" docs/ scripts/ Justfile CLAUDE.md ui/CLAUDE.md
grep -rn "check-doc-coverage.py:[0-9]" docs/ CLAUDE.md
grep -n "root" scripts/doc-coverage-allowlist.txt scripts/check-doc-coverage.py Justfile docs/dev/root-doc-maintenance.md
```

- **sweep 1 기대(열거형)**: 이력 plan 2파일만 잔존 — `docs/superpowers/plans/2026-07-30-claude-md-redistribute.md:267,269,374` + `2026-08-03-ui-claude-md-curation.md:47,104,220`(당시 plan-local 판정이 이 문자열에 의존했던 기록 — 수정 금지) + 이 spec 자신의 배경 서술. **그 외 0건**(특히 `scripts/`·`Justfile`·`docs/dev/` 0건).
...
| V3 (③ 단위) | 1글자 인라인코드가 낀 합성 텍스트로 `tokens()` 직접 호출(python -c, 스크립트 import) | 산문 조각이 토큰으로 추출됨 | 산문 조각 미추출 + 인접 실토큰 정상 추출 |
| V4 (회귀) | T1(스크립트+allowlist) 커밋 전 `just doc-coverage` (기본 f870cfd9/ui) | — | **OK** (C2에서 사전 실측 완료) |
| V5 (US3) | ⓐ §2.4 sweep 3종 = 열거형 기대치 일치 ⓑ 갱신된 검사명 인용으로 `grep -n "<검사명>" scripts/check-doc-coverage.py` 1회 도달 실증 ⓒ narratives :232 본문에 당시-제약 절 존재 | — | 전부 충족 |
...
| C9 | `base_root`/`cur_root` 사이트 13곳 | `grep -n "base_root\|cur_root" scripts/check-doc-coverage.py \| wc -l` → 13 |
