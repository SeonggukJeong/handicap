...
| R11 | `SHOULD` 라이브 검증(경량) — `getComputedStyle` 실측: h3 `fontSize 18px·fontWeight 600`·h4 `14px·#334155(rgb(51,65,85))`·이주 섹션 `marginBottom 24px`·워커 배지 `fontWeight 500` + 전/후 스크린샷. **셋업(리뷰 Fe2)**: ① ReportView 섹션군 = 완료 run 리포트 ② RunDetail 3섹션은 `terminal && report.data`의 **else 분기에만 렌더** → **실행 중(비-terminal) run**으로 검증(예: duration 60s+ run을 띄워 mid-flight에 `/runs/{id}` 조회 — `useRunReport`가 `enabled: terminal`이라 report.data 결정적 부재. terminal run은 failed여도 report가 200으로 오고[`build_report_for_run`에 status 게이트 없음] ReportView가 렌더돼 부적합; 죽은 타깃 URL도 per-request status-0으로 run이 정상 *완료*되므로 부적합) ③ 워커 배지는 도달 가능한 상태 1종 이상(pool-mode + drain 액션 → "드레인 중" 배지)을 실측, 미도달 상태(stale·ephemeral)는 RTL 클래스 집합 단언으로 갈음 | `/live-verify` 절차 기록(수치 포함) | |
