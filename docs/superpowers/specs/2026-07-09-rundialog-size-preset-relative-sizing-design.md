...
| R2 | MUST `sizing.ts`에 순수 함수 `sizePresetsFor(anchor: {vus,durationSeconds} \| null): {label,vus,durationSeconds}[]` 추가 — `anchor===null`이면 `ko.loadModel.sizePresets`(고정 3개)를 그대로 반환. | 단위 테스트 `sizePresetsFor(null)` → 기존 배열과 deep-equal | |
...
1. **B안(RunDialog가 앵커 계산 → prop) vs A안(LoadModelFields가 직접 hook 호출)** — 원래 A안(diff 최소)을 검토했으나, `LoadModelFields.test.tsx`는 `VuSizingHelper`/`SlotSizingHelper`/`WorkerSizingHelper`를 전부 `vi.mock`으로 스텁해 **`QueryClientProvider` 없이** 렌더된다. `usePriorClosedRunAnchor`는 내부에서 `useQuery`(via `useScenarioRuns`/`useRunReport`)를 호출하므로, `LoadModelFields`가 직접 이 hook을 부르면 `enabled:false`여도 `useQueryClient()`가 즉시 throw해 기존 스위트 전체가 깨진다. `RunDialog.test.tsx`는 RunDialog 자체가 이미 `useScenario` 등을 직접 써서 `QueryClientProvider`로 감싸져 있다(R9의 근거 — 기존 인프라를 그대로 활용). 그래서 앵커 계산 책임을 hook-호출 인프라가 이미 있는 RunDialog로 옮긴다(R5).
...
### 4.1 `components/VuSizingHelper.tsx` — 충족 R: `R1`
- `function usePriorClosedRunAnchor(...)` → `export function usePriorClosedRunAnchor(...)`.
...
- `import { formatDurationKo } from "../i18n/duration";` 추가.
- `const SIZE_PRESET_MULTIPLIERS = [0.5, 1, 2] as const;`
- `export function sizePresetsFor(anchor: Pick<ClosedRunAnchor, "vus" | "durationSeconds"> | null): { label: string; vus: number; durationSeconds: number }[]` — anchor null이면 `ko.loadModel.sizePresets`(import 필요) 그대로 반환, 있으면 배수 순회+클램프+중복 skip+라벨 조립.
...
- **VuSizingHelper의 기존 동작(캡션 "이전 run 기준" 등) byte-identical** — `usePriorClosedRunAnchor` 반환에 필드 하나(`durationSeconds`)만 추가, 기존 소비 코드는 그 필드를 안 읽음.
...
- **머지 전 필수**: `pnpm lint && pnpm test && pnpm build`(인자 없는 전체 `pnpm test` — targeted-green≠full-green 함정, ui/CLAUDE.md).
...
1. **4.1+4.2 (계약 없는 순수 로직)**: `VuSizingHelper.tsx` export+타입 확장 → `sizing.ts::sizePresetsFor` + 단위 테스트. 이 커밋만으로 기존 동작 무변화(아직 아무도 `sizePresetsFor`를 호출 안 함) — green 확인 후 다음.
