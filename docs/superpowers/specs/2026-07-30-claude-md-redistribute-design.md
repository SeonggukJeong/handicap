...
| R17 | **불릿 개수 불변식**(manifest 미선언 삭제 차단): 손댄 섹션의 불릿 수가 **`<base-ref>`에서 계산한 값 이상**이어야 한다(`current >= base`, 등호 아님). 기준값은 **하드코딩 금지** — `check-doc-coverage.sh`가 이미 받는 `<base-ref>`에서 매번 센다(오늘 값은 Subagent 27·ADR 인덱스 49). 정당한 불릿 **병합**은 manifest에 `kind=merge` 행으로 선언해 바닥을 그만큼 낮추되, **선언만으로는 안 되고 검증된다**: 그 행은 `absorbed_marker`(흡수되는 base 불릿에서 딴 짧은 verbatim 조각)와 `surviving_anchor`를 갖고, **현재 root에서 `surviving_anchor` 불릿이 `absorbed_marker`를 포함**해야 한다(아니면 `exit 1`). 진짜 병합했으면 흡수된 규칙의 조각이 살아남은 불릿 안에 있고, 그냥 지웠으면 없다. **`absorbed_marker` 자체도 검증한다** — base root에서 **정확히 1회** 출현해야 한다(흔한 조각을 고르면 무관한 이유로 살아남은 불릿에 이미 있어 검사가 통과한다). marker도 하나의 *선언*이므로 §9의 질문("그 선언은 무엇이 검증하나")을 한 단계 더 적용한 것이다. 크기 검사(≤250 B·≤6,144 B)는 삭제할수록 *더* 만족되므로 이 축이 없으면 조용한 삭제가 불가시다 |
...
**이 스크립트도 이빨을 증명해야 한다**(R11 시나리오 3건): ① L122를 목적지 기입 없이 지운 트리 ② manifest 행과 그 불릿을 함께 지운 트리 ③ `kind=merge`를 선언하고 실제로는 삭제만 한 트리 — 셋 다 RED가 안 나오면 스크립트가 틀린 것이다(②는 R17이, ③은 `absorbed_marker` 검증이 없으면 통과해버린다).
...
| `kind=merge`로 바닥만 낮추고 실제 병합은 안 함 | R17의 `absorbed_marker` 검증(+ base 유일성) + R11 시나리오 ③ |
...
**두 번째 관찰된 클래스(문서 위생)**: *정본 요구사항을 고치고 하위 재진술을 안 고친다.* 리뷰가 네 번 잡았다 — §4 예외가 R14를 못 따라감(N6) · R6가 R18 누락 · 서수 2차/3차 드리프트 · §3.2가 "시나리오 2건"으로 정체. 처방: **R-id를 편집하면 `grep -n '<R-id>'`로 전 재진술을 대조**한다(패치가 아니라 일괄). 이 문서가 다루는 splice 위험과 같은 뿌리다.
