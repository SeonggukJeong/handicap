...
| R6 | `scripts/check-doc-coverage.sh <base-ref>` + `just doc-coverage`: **manifest 기반 목적지-지향 검증**(§3.2) + **불릿 개수 불변식**(R17) + 보조 토큰 차분. 위반 시 exit 1 |
...
| R11 | **두 스크립트 모두** 이빨을 고의 회귀→RED→원복→GREEN으로 실증. coverage 쪽 **필수 시나리오 2건**: ① L122 불릿을 목적지 기입 없이 삭제 → RED ② **manifest 행과 해당 불릿을 함께 삭제 → RED**(②는 현 설계로는 통과 못 하므로 R17이 필요하다는 증거 — 리뷰 2차 Hole A) |
...
| R17 | **불릿 개수 불변식**(manifest 미선언 삭제 차단): 손댄 섹션의 불릿 수가 **`<base-ref>`에서 계산한 값 이상**이어야 한다(`current >= base`, 등호 아님). 기준값은 **하드코딩 금지** — `check-doc-coverage.sh`가 이미 받는 `<base-ref>`에서 매번 센다(오늘 값은 Subagent 27·ADR 인덱스 49). 정당한 불릿 **병합**은 manifest에 `kind=merge` 행으로 선언해 바닥을 그만큼 낮춘다. 크기 검사(≤250 B·≤6,144 B)는 삭제할수록 *더* 만족되므로 이 축이 없으면 조용한 삭제가 불가시다 |
| R18 | **baseline 인상의 기계적 조건**(§3.3b 래칫의 자기합리화 차단): 어떤 파일의 baseline이 `<base-ref>` 대비 올라갔으면 **그 파일의 실제 바이트가 `<base-ref>` 대비 줄었어야** 한다. 아니면 `exit 1`. 호스트는 `check-doc-coverage.sh <base-ref>`(base-ref를 받는 유일한 스크립트). **파일이 자라는 것은 경고(exit 0), baseline을 거짓 보고하는 것은 오류(exit 1)** |
...
| L2 | 지식 소실(옮겼다고 적었지만 실은 안 옮김) | `check-doc-coverage.sh`: manifest 목적지-지향(1차) + **불릿 개수 비감소 R17**(미선언 삭제) + 토큰 차분(3차) + **baseline 인상 조건 R18** | 영구 |
...
| 전체 | L4 프로브 ≥4문항 정답 도달 + 양성 대조 "모름" · R14 완료(0039 페이로드 3요소·0044 본문 2줄 주석·0047 오기 정정) |
...
없음. 사용자 확정 4건(범위=압축+재비대 방지 / 강제력=기계적 측정+구조 분리, pre-commit 차단 기각 / 목표 ~42–45 KB / US1 합격선 42 KiB) + 리뷰 1·2·3차 must-fix 16건·nice-to-have 14건 전건 반영(기각 0, N9는 측정 방식 병기).
