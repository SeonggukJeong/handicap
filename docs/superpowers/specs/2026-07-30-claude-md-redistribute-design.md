...
- 리뷰 이력: `spec-plan-reviewer` 1차 `APPROVE-WITH-FIXES`(must-fix 7·nice 6) → 2차 `APPROVE-WITH-FIXES`(must-fix 4·nice 4). **전건 반영, 기각 0.** 2차의 핵심 3건은 ① manifest가 *선언된 행*만 보므로 "선언을 잊으면" 여전히 불가시(→ R17 불릿 개수 불변식) ② `min_gain=0` 행은 항진명제(→ 표본 다항 marker + 한계 명시) ③ 예산 갱신이 현재치를 뒤따라가는 순환(→ §3.3 역할 분리). 반영 내역은 각 절에 `(리뷰 …)`로 표기.
...
| R14 | **ADR 신선도 해소**(대상 집합 `{0039,0044,0047}`는 13줄 전수 대조로 완결 — §1.6). ① `0044`: 헤더 한 줄로 부족하다 — **본문 `:19`·`:29`의 "no-op(슬라이스 3 연기)"·"(슬라이스 3) … 연기" 두 문장에 해소 주석**을 달아야 한다(안 그러면 §Decision을 읽은 사람은 여전히 *틀린 답*을 얻는다) ② `0039`: Status 최소 페이로드 = **구현 상태 + 머지 sha + build-log 단락 포인터**(인덱스가 갖고 있던 `bundle` feature off=byte-identical·서명/인스톨러 후속은 `docs/build-log.md`에만 있어 포인터 없이는 체인이 끊긴다) ③ root 0047 줄 `store`→`worker` 오기 정정. 그 후에만 §3.1 #4를 수행 |
...
| R17 | **불릿 개수 불변식**(manifest 미선언 삭제 차단): base 대비 `Subagent dispatch 노하우` 섹션 불릿 **27개**, `알아둘 결정들` 인덱스 불릿 **49개**가 유지돼야 한다. 규칙이 사라지면 개수가 준다 — 크기 검사(≤250 B·≤6,144 B)는 삭제할수록 *더* 만족되므로 이 축이 없으면 조용한 삭제가 불가시다 |
...
**#2의 두 상한은 거의 동시에 조인다** (리뷰 N4): 섹션은 불릿 27개(10,474 B) + 비불릿 506 B다. 250 B 초과 13개를 전부 정확히 250 B로 압축하면 섹션은 **~6,000 B**로, 6,144 B 상한과의 여유가 144 B뿐이다. 이 섹션에 새 산문(예: 새 목적지를 가리키는 포인터 문장)을 더하면 그 여유를 먹는다 — 포인터는 기존 머리말 문장을 고쳐 쓰고 새 줄을 늘리지 않는다.
...
**불릿 개수 불변식** (R17 — 리뷰 2차 Hole A): manifest는 *선언된 행*만 검사하므로, 불릿을 지우고 **manifest 행을 아예 안 만들면 어떤 검사도 발화하지 않는다**. 실패 모드가 "이관을 잊음"에서 "선언을 잊음"으로 한 칸 옮겨갔을 뿐이고 후자도 똑같이 불가시다(L134·L136·L138은 토큰이 0개라 2차 그물도 무력). → 손댄 섹션의 불릿 개수를 base와 대조한다(Subagent 27, ADR 인덱스 49). 규칙 하나가 사라지면 개수가 준다.
...
**10 KiB의 근거**(리뷰가 요구한 "쓰여진 도출 규칙"): 실측 성장은 controller ~750 B/커밋, ui ~1,483 B/커밋이므로 10 KiB ≈ 7–14 커밋 = 대략 한 분기. "분기마다 한 번 재분배를 검토한다"는 가독 가능한 주기이고, WARN이 울렸을 때 할 일(그 파일 재분배)이 명확하다. **baseline은 파일을 실제로 압축했을 때만 재설정한다** — 이 규칙을 `root-doc-maintenance.md`에 적어, 다음 인상이 "아무 기준 없음"이 아니라 *쓰여진 기준*을 상대로 논증하게 만든다.
...
| US | 검증 |
|---|---|
| US1 | `wc -c CLAUDE.md` ≤ **43,008** · L1 전 항목 통과 · L2 manifest 4검사 + **불릿 개수 불변식(27/49)** + 토큰 차분 통과 |
...
없음. 사용자 확정 4건(범위=압축+재비대 방지 / 강제력=기계적 측정+구조 분리, pre-commit 차단 기각 / 목표 ~42–45 KB / US1 합격선 42 KiB) + 리뷰 1·2차 must-fix 11건·nice-to-have 10건 전건 반영(기각 0).
