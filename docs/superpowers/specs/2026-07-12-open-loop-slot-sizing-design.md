...
실측 재현(2026-07-12, 100ms responder·target 20·15s)으로 확인된 결함: ① 엔진은 `target_rps`를 **반복(도착)/초**로 페이싱하는데(각 arrival = 시나리오 전체 1회 실행, 슬롯을 반복 내내 점유), 슬롯 권장 공식(`sizing.ts::recommendSlots`)과 사후 인사이트 `required`(`insights.rs:236-238`)는 **요청 1건의 평균 지연**만 반영한다 — 멀티스텝(2-스텝 run: dropped 40%)·per-step think time(think 1s run: 목표의 13%인 2.7 RPS로 plateau) 시나리오에서 체계적 과소 추천. ② 과소 추천된 `required ≤ max_in_flight`가 "슬롯 충분"으로 오판돼 `cause=loadgen` + 워커 증설 권장으로 빠지는데, open-loop fan-out은 `max_in_flight`를 **총량 분할**하므로(`api/runs.rs:695-696`→`shard_split`) 워커를 늘려도 RPS 불변(실측 2.7→2.7)·권장 워커 수만 발산(7→20). 올바른 공식(슬롯 = 목표 도착률 × 반복 점유시간)으로 돌리면 목표 도달(슬롯 23 → 19.9 RPS·dropped 0).
...
| R5 | MUST `Insight`에 `achieved_per_sec: Option<f64>`(달성 도착률, 포화 인사이트에서만 `Some`)를 additive로 추가한다 — serde `skip_serializing_if = "Option::is_none"`, UI Zod `.optional()`, `INSIGHT_COLUMNS` 13→14열 + 두 행-writer(`insight_csv_cells`/`write_insight_xlsx_row`) 갱신. | round-trip + `insight_columns_are_single_source` + export 테스트 | ✅ wire: UI Zod↔serde + CSV/XLSX 열 |
...
| R8 | MUST `SlotSizingHelper` 지연 앵커 우선순위를 재설계한다 — ⓐ prior 포화 open-loop run의 `load_gen_saturated`(cause=slots) `recommended`를 **그대로** 표시(공식 재계산 없음) → ⓑ prior 비포화 run: `iterationHoldMs`(per-step p50 = prior report `per_step`, fallback = `summary.mean_ms`; **결과 hold ≤ 0이면 앵커 무효 → 다음 순위**, localhost p50=0 가드) → ⓒ 수동 입력(의미를 "**반복 1회 예상 시간(ms)**"으로 변경) → ⓓ 측정 test-run: `apply_think_time: true`로 호출하고 `trace.total_ms`를 hold로 **직접** 사용(÷요청수 제거). | RTL: 4앵커 각 경로 + 우선순위 | |
| R9 | MUST(parity) 사전 권장 ≡ 사후 인사이트 — ⓐ 경로는 서버 `recommended`의 passthrough(by-construction)를 RTL로 단언하고, 곡선 `scheduled_arrivals` 적분은 서버 단위 테스트 fixture 숫자를 spec §4.1에 기록해 검증을 고정한다. | RTL passthrough + 서버 fixture 단언 | |
...
| R11 | MUST 무변경 불변식 — 엔진(`runner.rs`)/proto/migration/`openLoopChecks.ts`/`validate_run_config` 0-diff; closed-loop 인사이트 경로·`dropped=0` run 리포트는 byte-identical(신규 필드 R5는 포화 인사이트에만 실림); `sut_stress`·`onset_second`·`slowest_step` 등 여타 인사이트 무변경. | 기존 스위트 green + `no_saturation_when_dropped_zero` 유지 | |
| R12 | MUST ko 문구(`ko.ts` saturation/슬롯·워커 헬퍼) — cause=slots는 "목표 도착 X/s 중 Y/s만 시작(초당 Z건 유실) → max_in_flight를 N 이상으로", cause=sut는 "서버 응답 열화 신호 — 부하·슬롯 증설 보류, 서버 지표 확인"을 명시하고, 곡선 run의 권장값엔 "(피크 기준 상한 추정)"을 병기한다 — 수치·행동이 문장 안에 있어 초보자도 다음 행동을 바로 안다. | RTL 문구 단언 + 라이브 육안 | |
| R13 | MUST 경계 가드 — `achieved_arrival_rate ≤ 0`(dropped ≥ scheduled)이면 `recommended = 10_000`(validate 상한)으로 클램프하고 상한 도달 문구를 표시한다; `recommended`는 항상 `min(계산값, 10_000)`. | 단위: 클램프 케이스 | |
...
3. **사전 권장 == 사후 인사이트 불변식은 passthrough로 격상(R8ⓐ/R9)**: 기존엔 UI가 서버와 "같은 수식"을 복제해 parity를 유지했는데(드리프트 위험), 포화 prior run에선 서버가 이미 정확한 `recommended`를 계산해 두므로 UI는 **재계산 없이 그대로 표시**한다 — 수식 복제 1곳 제거, parity가 by-construction.
...
5. **`achieved_per_sec` 필드 추가(R5)가 초보자-자명 문구(R12)의 전제**: "목표 X/s 중 Y/s만 시작"을 쓰려면 달성 도착률이 인사이트에 실려야 한다. `value`(관측 peak, 요청/초)는 기존 소비처 보존을 위해 의미 무변경.
...
- `derive_insights`의 `dropped > 0` 블록(현 213-271행) 재작성:
  - `scheduled_arrivals(profile 파생 인자, duration_actual)`: 고정 = `f64(target) × duration`; 곡선 = `Σ_stage 사다리꼴(prev_target→target, dur)`을 `duration_actual`에서 절단(마지막 부분 stage는 선형 보간 값까지 적분). **입력은 `report.rs` 호출부가 전달**(아래 4.2) — `derive_insights`에 11번째 인자 `scheduled_arrivals: Option<f64>` 추가(`#[allow(clippy::too_many_arguments)]` 기존).
  ...
  - cause: `sut_stress(...)` → `("sut", recommended=None)`, else `("slots", recommended)` (R3). loadgen arm·`recommended_workers` 계산 블록 삭제(R4).
  - `ins.achieved_per_sec = Some(achieved_rate)` (R5). `value`(peak)·`count`(dropped)·`onset_second` 무변경.
...
- `Insight` struct: `pub achieved_per_sec: Option<f64>`(serde `default, skip_serializing_if`), `recommended_workers`에 deprecated 주석.

### 4.2 `crates/controller/src/report.rs` — 충족 R: R2
- `derive_insights` 호출부(현 767행 부근): 기존 `target_eff` 도출(776-780행, `target_rps.or_else(stages peak)`) 옆에서 `scheduled_arrivals`를 계산해 전달 — profile(`run.profile`)의 `target_rps`/`stages`와 `summary.duration_seconds` 사용. open-loop이 아니면(둘 다 없음) `None`.

### 4.3 `crates/controller/src/export.rs` — 충족 R: R5
- `INSIGHT_COLUMNS` 13→14열(`achieved_per_sec` 추가) + `insight_csv_cells`/`write_insight_xlsx_row` 각 한 줄 — 4표면 동시 반영(기존 단일-소스 계약).

### 4.4 `ui/src/api/schemas.ts` — 충족 R: R5
- `InsightSchema`(또는 해당 인사이트 Zod)에 `achieved_per_sec: z.number().optional()` — 서버 `skip_serializing_if`라 absent→`.optional()`(B7-C 3종 분기 규칙).
...
- 앵커 재설계(R8 ⓐ→ⓓ). ⓐ: prior report `insights`에서 `kind=load_gen_saturated && cause=slots && recommended>0` → 그 값을 권장으로 직접 표시("직전 run 실측 기반") — `recommendSlots` 미호출. ⓑ: `iterationHoldMs`(prior report `per_step`의 `p50_ms` 맵, fallback `summary.mean_ms`) → `recommendSlots`. ⓒ: 수동 입력 라벨·placeholder를 "반복 1회 예상 시간(ms)"으로. ⓓ: `testRun.mutate({..., apply_think_time: true})` + `trace.total_ms`를 hold로.
...
### 4.8 `ui/src/i18n/ko.ts` + `InsightPanel` 소비처 — 충족 R: R12
- `ko.insights.saturation` cause 분기 문구 재작성(2-way): slots = 목표/달성 도착률·유실률·권장 슬롯 수치 포함, sut = 증설 보류 경고. loadgen 문구 키 제거(소비처 분기도 함께). 곡선 병기 "(피크 기준 상한 추정)". 조사 병기 규칙((으)로) 유지.
...
1. **계약-먼저(R5)**: `Insight.achieved_per_sec` serde + UI Zod `.optional()` + `INSIGHT_COLUMNS` 14열 — 한 green 커밋(와이어 양쪽 동시).
2. **서버 공식(R1-R4, R13)**: `insights.rs` 재작성 + `report.rs` scheduled 전달 + 단위 테스트(fixture §4.1) — 기존 loadgen 테스트 갱신 포함 green 커밋.
