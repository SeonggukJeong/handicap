...
* Status: 완료 (9a 엔진 + 9b UI authoring + 9c 상호 1레벨 중첩 + 9d 분기별 메트릭 breakdown 구현·머지 완료 — ADR-0023)
* Date: 2026-05-30
...
- `all`/`any`/`left` 어느 키도 없는 malformed 맵(예: `{op: eq}`)은 `de::Error::custom`으로
  거부(`Body` visitor의 unknown-field 에러와 동일 처리). 세 모양은 `Compare`가 `left`를 필수로
  가져 `all`/`any` 키와 절대 겹치지 않으므로 모호하지 않다.
...
- `right: None`은 `exists`/`empty`에서만 정상(그 둘은 `right` 무시). 그 외 op에서 `right`가 없으면
  렌더값을 빈 문자열 `""`로 취급한다(lenient 일관 — 별도 에러 없음).
...
**`Step::If` arm은 들어온 `ctx`(그 안의 `loop_index`)를 분기 자식에 그대로 전달**한다 — 새 스코프를
만들지 않는다. 빠뜨리면 if-in-loop에서 분기 안 http가 `loop_index`를 잃는다.
...
- `StepModel`은 현재 `z.discriminatedUnion("type", [Http, Loop])` — `IfStepModel`(`type:"if"`) 추가는 그대로 OK.
- 게이트(§5): `LoopStepModel.do` → http+if, `IfStepModel`의 then/elif[].then/else → http+loop. (9b는 분기 http-only로 시작, 9c에서 게이트 완화.) **주의**: 9c 게이트 완화는 `StepModel`을 자기참조시켜 **`z.lazy` 상호재귀를 새로 도입**한다(현 Zod 모델은 lazy 전무 — `ConditionModel` 외 첫 사용처). discriminatedUnion+lazy+Zod default 누출 조합의 `tsc` 함정 주의(ui/CLAUDE.md).
...
- **`flattenHttpSteps`**: 현재 구현은 **비재귀**(`out.push(...s.do)` — loop 바디를 `HttpStep[]`로 가정). 9b(분기 http-only)는 if 분기 3종(then/elif[].then/else)을 walk하도록 한 단계 확장이면 충분하나, 9c 중첩(loop-in-if/if-in-loop)에선 `s.do`·분기가 `Step[]`(loop·if 포함)가 되므로 **진짜 재귀로 재작성**해야 한다(반환 타입 `HttpStep[]`는 불변 유지). report 라벨링·inspector·data-binding 스캔이 모두 이 헬퍼를 재사용하므로 한 곳만 고치면 전파.
...
엔진 Aggregator: per-(if_id, branch) count
  branch ∈ { then, elif_0, elif_1, …, else, none }   # none = 무매치 & else 없음/빈
      ...
      → worker가 MetricFlush → proto MetricBatch 변환 (crates/worker/src/main.rs, loop_stats 변환부 옆)
      → gRPC MetricBatch.branch_stats
      → controller run_if_metrics 테이블 (migration 0006, CREATE TABLE IF NOT EXISTS 멱등)
        UPSERT-accumulate
...
- **`run_if_metrics` 스키마는 7-1의 단일 정수 `loop_index`와 동형이 아니다** — branch가
  가변 길이(`then`, `elif_0…elif_n`, `else`, `none`)라서. 결정: branch를 **TEXT 키**
  (`"then"`/`"elif_0"`/`"else"`/`"none"`)로 저장(정수 인코딩보다 가변 elif·미래 확장에 안전),
  PK `(run_id, if_id, branch)`. 7-1 `u32::MAX` overflow sentinel 패턴은 차용 안 함.
- **`MetricFlush.branch_stats` 추가는 worker 변환 hop을 동반**: 엔진의 모든 `run_scenario`
  consumer 컴파일을 건드리고(엔진 CLAUDE.md), worker `MetricFlush → MetricBatch` 변환부
  (`crates/worker/src/main.rs` loop_stats 변환 옆)와 prost exhaustive literal 전 사이트 수정이
  필요하다(`..Default::default()` 금지 — controller CLAUDE.md). proto `branch_stats`는 새 field 번호.
- `runs` 테이블 무변경(프로파일은 `profile_json`).
...
- Zod(`ConditionModel` `z.lazy` 재귀 + `IfStepModel` + elif), `StepModel` discriminatedUnion에 if 추가, `flattenHttpSteps`를 if 분기 walk하도록 확장(아직 비재귀로 충분 — 분기 http-only).
...
### 9c — 상호 컨테이너 중첩 (Y)
- Zod 게이트 완화: `loop.do` → http+if, `if` 분기 → http+loop. **`StepModel`을 `z.lazy` 상호재귀로 전환**(현 모델 첫 lazy 사용 — Zod default 누출 `tsc` 함정 주의).
- **`flattenHttpSteps`를 진짜 재귀로 재작성**(SF-1: 현재 `push(...s.do)` 비재귀라 `Step[]` 중첩에서 타입이 깨짐). 반환 타입 `HttpStep[]` 불변.
- 캔버스 중첩 subflow depth 렌더(두 방향).
- 테스트: 중첩 round-trip, 캔버스 중첩 렌더 RTL.

### 9d — 분기 메트릭 breakdown
- 엔진 `Aggregator` per-(if_id, branch), `MetricFlush.branch_stats`(모든 `run_scenario` consumer + worker `MetricFlush→MetricBatch` 변환부 수정 — SF-3).
- proto `MetricBatch.branch_stats`(새 field 번호, prost exhaustive literal 전 사이트), controller `run_if_metrics`(migration 0006, branch는 TEXT 키 — SF-4), `ReportStep.branch_breakdown`.
