...
- **날짜**: 2026-07-12
- **상태**: 설계 승인(사용자 2026-07-12, 프로토타입 전/후 실측 확인) → spec 리뷰 대기
...
|---|---|---|---|
| R1 | MUST — `safeDecodeUrl(url)`은 URL의 경로·쿼리 구간에서 연속 `%XX` run을 UTF-8로 디코딩하되, **허용 집합**(① 비ASCII 가시 문자 = `\p{C}`·`\p{Z}` 제외, ② ASCII unreserved `A-Za-z0-9-._~`, ③ 공백 0x20)에 드는 문자만 디코딩 확정 | unit: 한글 경로/쿼리·`%20`·`%41%42%43` 디코딩 golden | 순수 함수 |
| R2 | MUST — 허용 외로 디코딩되는 escape(구조 `%25 %26 %3D %2B %23 %2F %3F` 등·템플릿 `%7B %7D %24`·제어/비가시 문자)는 인코딩 보존하며, 판정은 run 내 **문자 단위**(혼합 run은 부분 디코딩: `%EA%B9%80%26%EC%9D%B4`→`김%26이`); 보존 escape의 hex 대문자 정규화는 허용(의미 동일) | unit: 중첩 URL(`redirect=https%3A%2F%2F…`) 불변·템플릿 문자 불변·혼합 run golden | |
...
| R7 | MUST — Referer/Origin 헤더 값(`parameterizeRefHeaders`)·form/JSON 바디·`filters.ts`(dedupKey/isStaticAsset/entryHost — 원문 기준)는 무변경 | 기존 테스트 green + Referer 값 불변 신규 단언 | |
| R8 | MUST — 디코딩된 URL(한글·공백 포함)이 든 생성 YAML은 `parseScenarioDoc` green(에디터 게이트 통과) | golden 테스트에서 생성 YAML `parseScenarioDoc` 단언 | |

---

## 3. 핵심 통찰 (설계 근거)

1. **엔진 실측 — 전송 의미 불변의 근거**: `crates/engine/src/executor.rs:137-158`은 렌더된 URL 문자열을 reqwest(`url` crate = WHATWG URL 파싱)에 그대로 넘긴다. WHATWG 파서는 경로·쿼리의 비ASCII를 UTF-8 percent-이스케이프로, 공백을 `%20`으로 재인코딩한다 → 허용 집합 ①③은 **와이어 바이트가 디코딩 전과 동일**, ②(unreserved)는 raw 전송이지만 RFC 3986 §2.3이 인코딩형과 동등 규정. 설계 승인 시 Node WHATWG `new URL`(같은 스펙)로 10케이스 전/후 `href` 동일을 실측했다(스크래치 프로토타입, 2026-07-12).
...
- run → 바이트 배열 → `TextDecoder("utf-8", { fatal: true })`; 실패 시 run 원문 반환(R4).
- 문자별 판정: 코드포인트 < 0x80이면 `[A-Za-z0-9\-._~ ]`, 아니면 `!/[\p{C}\p{Z}]/u`(R1). 비허용은 해당 문자의 UTF-8 바이트를 `%XX`(대문자)로 재출력(R2).
...
- 신규 `ui/src/import/__tests__/urlDecode.test.ts`: §2 acceptance의 unit 전부(한글/공백/unreserved·중첩 URL·템플릿 문자·혼합 run·깨진 입력·authority/fragment·`${VAR}`·멱등).
- `harToScenario.test.ts`: 인코딩 entry golden(디코딩 URL·이름, hostVars on/off)·Referer 값 불변·생성 YAML `parseScenarioDoc` green(R8).
- `ScenarioImportPage.test.tsx`: 인코딩 HAR 업로드 → 미리보기 행이 디코딩 표시.
...
3. **라이브 검증(필수 취급)**: 엔진 코드는 0-diff지만 엔진이 소비하는 URL 데이터 형태가 바뀌므로 와이어 실증 1회 — 한글 쿼리·경로가 든 HAR을 UI로 가져와 시나리오 생성 → 로깅 echo responder(`print(f"REQ {self.command} {self.path}")` 변형 — 루트 CLAUDE.md: 번들 responder는 no-op)로 test-run 또는 run 실행 → 와이어 REQ 라인에 `%ED%95%9C%EA%B8%80` 등 **재인코딩된 바이트 도달**을 grep 실증. 가능하면 같은 HAR의 디코딩-전 시나리오(이 기능 이전 형태)와 REQ 라인 동일 비교.
