...
- `harToScenario.test.ts`: 인코딩 entry golden(디코딩 URL·이름, hostVars on/off)·**`%XX` 든 Referer 값 인코딩 유지**(리뷰 1R #1 — ASCII-only fixture는 vacuous)·생성 YAML `parseScenarioDoc` green + R8 ⓐⓑ 인용 분기 golden.
