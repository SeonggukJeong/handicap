...
**flush send-guard 2곳도 갱신**(spec-reviewer 발견 — 4 리터럴 사이트와 별개): 두 flush 사이트는 "보낼 데이터 있나" 가드로 감싸여 있어 group_stats만 차 있고 windows/loops/branches가 비면 flush가 **스킵돼 그룹 샘플 유실**된다. parallel 시나리오는 http-leaf 윈도가 항상 차 있어 실제로는 안 터지나, `dropped`가 `&& flush.dropped == 0`을 넣은 것과 같은 방어로 `|| !groups.is_empty()`(또는 동등)를 추가한다:
- closed-loop final 가드 `runner.rs:228`: `if !final_windows.is_empty() || !final_loops.is_empty() || !final_branches.is_empty()` → group 조건 추가.
- open-loop periodic `has_data` 가드 `runner.rs:615`: `!drained.is_empty() || !loop_stats.is_empty() || !branch_stats.is_empty()` → group 조건 추가.

(closed periodic `:195`·open final `:771`은 무조건 보내므로 가드 없음 — 위 2곳만.)
...
// ReportSchema 에:  group_latency: z.array(GroupLatencySchema),   // plain, NOT .default([])
```
서버가 항상 배열(빈 `[]` 포함)을 보내므로 **plain required `z.array(...)`** 를 쓴다 — `dropped: z.number()`와 같은 plain 패턴. **`.default([])` 금지(spec-reviewer)**: ui/CLAUDE.md(S-C) 함정 — 응답 스키마의 top-level `.default()`는 `request<T>`로 `T | undefined`를 누출시키고 `tsc -b`(`pnpm build`)에서만 잡힌다. (`if_breakdown`/`insights` UI 측도 `.optional()`/plain이지 `.default([])` 아님.) 그리고 **머지 전 라이브 run으로 실제 응답이 배열인지 확인**(S-D 교훈 — RTL absent fixture는 서버 실제 shape를 안 보장).
