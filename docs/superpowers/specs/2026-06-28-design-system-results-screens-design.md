...
- **날짜**: 2026-06-28
- **상태**: 설계 (spec-plan-reviewer 검토 대기)
...
- **목표**:
  1. 블록-레벨 알림(오류/경고/상태 박스)을 **기존 프리미티브 `Callout`으로 드롭인 교체**(룩·역할 1:1 보존).
  2. 표시 화면의 정당한 raw `<input>`(WorkerDashboard PromptDialog)을 `Input`으로 통일(포커스 링·`aria-invalid` 획득).
...
| R3 | `MUST`(불변식) **동작 byte-identical** — 핸들러·react-query 훅·도출(`useMemo`/필터/정렬/라벨)·상태 round-trip이 재구성 전과 동일. JSX 마크업만 교체 | 각 페이지/컴포넌트 기존 테스트 전부 통과; 로직 함수 0-diff(리뷰) | |
| R4 | `MUST` **블록-레벨 알림 → `Callout`**, **기존 `role` 1:1 보존**(없으면 안 만든다): `role="alert"`(blocking 오류)→`Callout role="alert"`·`role="status"`(non-blocking 경고)→`Callout role="status"`·roleless 경고 박스→`Callout` role 미부여. variant = 빨강 박스→`error`·호박 박스→`warn` | 전환 박스 role이 전환 전과 1:1; variant 색이 기존 박스 색과 매칭; 기존 role RTL 통과 | |
...
| R8 | `MUST`(불변식·동결) **`InsightPanel`·`InsightCompareMatrix` severity 색 맵**(`critical: border-red-300 bg-red-50 text-red-800`·`warning: border-amber-300 ...`·`info`)은 **`Callout`으로 변환하지 않는다** — Callout variant(error=`red-200/red-700`·warn=`amber-300/amber-800`)와 **색조가 달라**(red-300/red-800) 변환 시 시각 회귀 + severity는 자체 3단계(critical/warning/info) 팔레트(별 도메인) | 두 파일 severity 맵·렌더 0-diff(grep) | |
...
| R11 | `MUST`(불변식) accent 드리프트 — 대상 파일에 `text-blue-*`/`bg-blue-*`/`indigo-*` 컨트롤 색 **드리프트 0개**. 새 컨트롤 색을 들이지 않는다(C-2가 이미 수렴) | 만진 파일 blue/indigo 드리프트 0(grep) | |
...
4. **변환 경계 규칙 = "tinted 알림 박스 + role=alert 오류 텍스트"만**(R4·R5). 빨강(`bg-red-50`)·호박(`bg-amber-50`) 박스와 `role="alert"` 오류 텍스트(borderless 빨강 포함 — design-system-spread가 list-load `text-red-600` 오류를 Callout화한 선례)는 Callout로 통일하되, **색 없는(slate) 로딩/상태 텍스트·slate 박스·버튼 옆 인라인 span은 유지**한다 — `Callout`엔 plain/slate variant가 없어 박스화·재색이 시각 회귀이기 때문. 이 경계가 "통일(좋음)"과 "회귀(나쁨)"를 가른다.
...
- 목록 로드 오류 `<p role="alert" className="mb-3 text-sm text-red-600">`(`:37`) → `Callout variant="error" role="alert"`(borderless 빨강 오류 → 박스 통일, design-system-spread list-load 선례). children/문구 ko 그대로.
- **EmptyState CTA(`:43`)는 공유 컴포넌트라 동결**(R7 정신 — 시그니처 무변경).
- ⚠ **`ScenarioListPage`는 기존 테스트 파일이 없다**(`ui/src/pages/__tests__/ScenarioListPage.test.tsx` 부재) → **신규 focused 테스트 추가**(오류 시 `getByRole("alert")` Callout 렌더 + `EmptyState` 분기)가 회귀 가드 겸 tdd-guard pending diff. 나머지 7개 파일은 기존 테스트 존재(lockstep).

### 4.2 ScenarioRunsPage + (RunListControls 동결) — 충족 R: `R1,R3,R4,R5,R7,R9,R13`
- 페이지 오류 박스 `role="alert" border border-red-200 bg-red-50 text-red-700`(`:183`) → `Callout variant="error" role="alert"`.
- **버튼 옆 인라인 `<span role="alert" text-red-600>`(`:277`)은 인라인 유지**(R5 — 전환=레이아웃 시프트).
...
- 오류 박스 `role="alert" border-red-200 bg-red-50 text-red-800 rounded`(`:181`·`:189`·`:236`·`:249`) → `Callout variant="error" role="alert"`(공통 4곳).
...
- 오류 박스 `role="alert" bg-red-50 text-red-700`(`:46`·`:116`[PromptDialog 내]·`:426`[flex justify-between]) → `Callout variant="error" role="alert"`. `:426`는 flex 레이아웃 `Callout className=`으로 보존.
- borderless 오류 `<p role="alert">{ko.workers.loadError}</p>`(`:402`) → `Callout variant="error" role="alert"`.
- **PromptDialog 입력 `<input type={inputType} w-full rounded border-slate-300 px-2 py-1.5>`(`:106`) → `<Input className="mb-2" aria-label={title} type={inputType} value onChange />`**(R6 — 이미 `w-full`·`mb-2`만 className·focus/invalid 획득).
- **slate 로딩 텍스트 `<p role="status" text-slate-500>`(`:398`)·busy/idle/stale 상태 배지(`STATUS_STYLE` 계열) 동결**(R5·R7 — 데이터-식별 상태 색; stale 배지 roleless 컨벤션은 `ui/CLAUDE.md` 워커 대시보드 항목).
...
- 리포트 로드 오류 박스 `role="alert" border-red-200 bg-red-50 text-red-700`(`:147`) → `Callout variant="error" role="alert"`.
...
1. **`ScenarioListPage`** (작고 드롭인 1곳·낮은 위험 = 시작점) — 목록 오류 → Callout. **기존 테스트 없음 → 신규 focused 테스트 추가**(오류 Callout 렌더 + EmptyState 분기 = pending diff + 회귀 가드, F1).
2. **`ScenarioRunsPage`** — 페이지 오류 박스 → Callout(인라인 span·stall/status 배지·checkbox·`RunListControls` 동결). 기존 `ScenarioRunsPage.test` lockstep(필터 칩 ↔ verdict 배지 disambiguate는 `ui/CLAUDE.md` 항목 유지).
3. **`RunDetailPage`** — 오류 4곳·경고 2곳 → Callout(slate 로딩 박스·배지·차트 동결). flex 박스(`:205`) className 보존. 기존 `RunDetailPage.test` lockstep.
4. **`WorkerDashboardPage`** — 경고/오류 박스 → Callout(roleless/role 보존)·PromptDialog 입력 → Input(slate 로딩·status 배지 동결). 기존 테스트 lockstep + PromptDialog 입력/경고 단언(F1).
