...
| R3 | MUST: 선언 행에서 `flatExtractNames.has(name)`이면 nameline에 amber 배지(`rounded bg-amber-50 px-1.5 text-xs text-amber-700` — 기존 "분기" 배지의 amber 판) + `title` 설명을 표시하고, 아니면 배지 부재 | RTL: 선언+flat extract 동명 fixture에서 배지 텍스트 존재+title 확인·extract 없는 선언 행 배지 부재 | |
| R4 | MUST: 선언 변수가 **parallel 분기에서만** 추출되는 이름이면 amber 배지를 달지 않는다(네임스페이스 merge라 덮어쓰지 않음 — 이름 혼동은 parallel 행 shadow 배지가 기존대로 담당) | RTL: 선언+parallel-only extract fixture에서 amber 배지 부재 + parallel 행 shadow 배지 기존 단언 green | 엔진 merge 의미론(runner.rs:638)과 1:1 |
| R5 | MUST: ⚠ 미정의 행에 텍스트 링크 버튼 "선언 추가"(usage 버튼과 같은 `text-xs text-accent-600 hover:underline` 이디엄, `aria-label`은 이름 포함) — 클릭 시 `setVariable(name, "")` 1회 호출로 행이 선언 행으로 전이(빈 값 textarea) | RTL: 클릭 후 ⚠ 행 소멸·선언 행(값 textarea·× 버튼) 등장·YAML에 `name: ""` 반영 | |
| R6 | MUST: "선언 추가" 버튼은 `yamlError !== null`이면 disabled(연필 버튼과 동일 게이트 — store dispatch no-op의 시각적 미러) | RTL: yamlError 상태에서 `toBeDisabled` | |
...
1. **flat extract만 배지 대상인 이유 (정확성)**: 엔진 우선순위는 선언 seed → (데이터셋 주입) → **flat extract 덮어쓰기**. parallel 분기 extract는 join 후 `분기명.변수` 네임스페이스 키로만 merge되어 bare 선언값을 건드리지 않는다 — parallel 충돌에 amber "덮어씀" 배지를 달면 **거짓말**(실제로 안 덮어씀)이고, 그 클래스는 이미 shadow 배지("이름 충돌", rename 비활성 근거)가 정확한 의미로 담당한다. 경고 문구가 런타임 사실과 1:1이어야 한다는 원칙(resolveForDisplay "진단 표시가 거짓말" 함정과 같은 정신).
...
- `rows` useMemo: `const flatEx = flatExtractNames(model)` 추가, declared 행에 `overwritten: flatEx.has(name)` 필드.
...
- 미정의 행: ⚠ 배지 뒤에 `<button type="button" aria-label={ko.editor.variableDeclareAddAria(row.name)} disabled={yamlError !== null} onClick={...}>` — `setUsageNav(null)` 후 `setVariable(row.name, "")`. className `shrink-0 text-xs text-accent-600 hover:underline disabled:opacity-40`.

### 4.3 `ui/src/i18n/ko.ts` — 충족 R: R10

- `variableOverwritten: "추출 덮어씀"`, `variableOverwrittenTitle: "스텝 추출이 같은 이름에 값을 씁니다 — 추출 스텝 이후엔 선언값 대신 추출값이 사용됩니다"`, `variableDeclareAdd: "선언 추가"`, `variableDeclareAddAria: (name: string) => `${name}을(를) 변수로 선언`` (기존 `variable*` 키 군집 옆).
...
- 기존 행 4종의 문구·shadow 배지·rename/usage 팝오버 동작: byte-identical(선언 행 nameline 클래스만 R9로 변경 — 배지 없으면 시각 no-op).
- run 생성/리포트/test-run 경로: 무접촉 — **라이브 검증은 클라이언트-only**(`/scenarios/new`, 백엔드 불요).
- 하단 "추가" 버튼 경로·검색·varsWide/wideOpen 그리드 계약: 무변경.
...
| R3·R4 | RTL: 동명 fixture 배지 존재/부재 3케이스(flat 충돌·무충돌·parallel-only) | |
| R5·R7 | RTL: 클릭 → 행 전이·YAML 반영(일반 이름 + 점 포함 이름) | |
| R6 | RTL: yamlError 상태 disabled | |
| R8 | RTL: 팝오버 열림 → 클릭 → 소멸 | |
