...
| R6 | MUST `max_requests` 예산과 wall-clock 120s deadline을 모든 행에 걸쳐 단일 예산으로 공유 — 소진 시 그 지점에서 중단하고 top-level `truncated=true`, 미실행 행은 응답 `rows`에 없음(마지막 행은 mid-cut 가능 → 그 행의 `trace.truncated=true`); **경계**: 계획된 반복(R18의 N)을 전부 완료했으면 마지막 행 종료와 동시에 예산이 소진돼도 `truncated=false` | engine 단위: 예산 소진 케이스(행 경계·행 중간·정확 소진) 단언 | |
...
| R18 | MUST 서버가 sequential 반복 수를 `N = min(row_limit ?? (첫 바인딩 row_count − start_row), 첫 바인딩 row_count − start_row, max_requests)`로 clamp하고, 행 로드는 바인딩별 ≤ `min(len_k, N)`행(연속 range fetch 1–2회 — 전체 데이터셋 선로드 금지); clamp로 계획 구간을 못 돌면 `truncated=true`(0-http-leaf 시나리오도 이 clamp로 행 수·응답 크기 유계) | controller/engine 단위: 1M-행 메타 가정 clamp·0-leaf 시나리오 유계 단언 | |
