...
| R3 | MUST 바인딩의 `mappings` 생략/빈 배열이면 서버가 모든 컬럼→동명 변수로 자동 매핑하고, 명시 `mappings`는 자동을 완전 대체 | controller 단위: 자동/명시 각 케이스의 시드 결과 단언 | |
| R4 | MUST 주입 세만틱이 run을 미러: `iter_vars = scenario.variables + 행 매핑 결과`(충돌 시 데이터셋 우선), 행마다 iter_vars 리셋(추출 비누적), `vu_id=0`, `iter_id=행 순번`(single_row=0, sequential=0..N-1) | engine 단위: 시드·리셋·iter_id 단언(run 쪽 `run_vu` 시드 순서와 대조 주석) | |
| R5 | MUST sequential 모드에서 cookie jar(VuClient)를 행 간 공유(클라이언트 1회 빌드 — 1 VU 순차 세만틱) | engine 단위: set-cookie 후 다음 행 요청에 쿠키 실림 단언 | |
| R6 | MUST `max_requests` 예산과 wall-clock 120s deadline을 모든 행에 걸쳐 단일 예산으로 공유 — 소진 시 그 지점에서 중단하고 top-level `truncated=true`, 미실행 행은 응답 `rows`에 없음(마지막 행은 mid-cut 가능 → 그 행의 `trace.truncated=true`) | engine 단위: 예산 소진 케이스(행 경계·행 중간) 단언 | |
| R7 | MUST `single_row` 응답은 기존 `ScenarioTrace` 형태 그대로(신규 필드 0 — UI 렌더 경로 무변경, 주입 값은 `final_vars`로 관찰 가능) | curl: 응답이 기존 `ScenarioTraceSchema`를 통과 | ✅ wire: 기존 응답 보존 |
...
| R9 | MUST 검증 실패는 422 한국어 메시지: ① dataset_id 미존재 ② 명시 매핑의 컬럼 미존재 ③ `row_index` ≥ 해당 바인딩 row_count / `start_row` ≥ 첫 바인딩 row_count(N 산식 기준 — R17) ④ `row_limit < 1` ⑤ 바인딩 간 변수명 중복(기존 `collect_var_names` 재사용) ⑥ `bindings` 빈 배열 ⑦ `single_row`인데 `row_index` 누락 ⑧ `sequential`인데 `row_index` 지정 | controller 단위 8케이스 + curl 대표 케이스 | |
| R10 | MUST sequential은 실패 행에서도 끝까지 진행(fail-fast 없음 — 중단 조건은 R6 상한뿐) | engine 단위: 실패 행 뒤 행도 실행됨 단언 | |
| R11 | MUST TestRunSection에 접이식 "데이터셋" 섹션(기본 접힘, 구성된 채 접히면 요약 힌트 — 기존 optional 섹션 이디엄): 데이터셋 Select·모드 라디오(특정 행/순차 검증)·행 선택·순차 파라미터(시작 행·행 수, 비움=전체)·매핑 요약+펼침 편집(Column만) | RTL: 섹션 렌더·구성→요청 페이로드 매핑 단언 | |
...
| R17 | MUST 멀티 바인딩 세만틱(와이어 계층): sequential에서 바인딩 k의 행 = `(start_row + i) % len_k`, 반복 수 N = `row_limit` 지정 시 그 값, 생략 시 `첫 바인딩 row_count − start_row`; single_row는 바인딩별 `row_index` | engine/controller 단위: 2-바인딩 케이스 단언(UI 노출은 §7 연기) | |
...
4. **자동 same-name 매핑은 서버 소유(R3)**: 클라가 자동 매핑을 계산해 명시 mappings로 보내면 curl 사용자·후속 클라이언트마다 재구현이 필요하고 드리프트 위험. 서버가 소유하면 "빈 mappings = 전 컬럼 동명 주입"이 계약이 된다.
...
- 예산 배선: 기존 `TraceState.requests` 카운트를 행 간 이월(다음 행 시작 전 잔여 0이면 중단·top-level truncated).
...
- `TestRunRequest`에 `#[serde(default)] dataset: Option<TestRunDatasetConfig>`:
  ...
  pub struct TestRunDatasetConfig {
      ...
      pub bindings: Vec<TestRunBinding>,     // 비면 422 (R9-⑥)
      #[serde(default)] pub start_row: u64,  // sequential 전용
      #[serde(default)] pub row_limit: Option<u64>, // sequential 전용, None=전체
  }
  pub struct TestRunBinding {
      pub dataset_id: String,
      #[serde(default)] pub mappings: Vec<Mapping>, // binding.rs 재사용 (R2)
      #[serde(default)] pub row_index: Option<u64>, // single_row 필수 (R9-⑦/⑧)
  }
  ```
- 핸들러: dataset 존재/컬럼/범위/중복 검증(R9, 기존 `collect_var_names` 재사용) → `get_rows_range`로 필요 행 로드 → 자동 또는 명시 매핑을 `apply_mappings`로 적용(R3, US4 패리티) → single_row는 시드 1행으로 trace 후 기존 `Json<ScenarioTrace>` 반환(R7), sequential은 `trace_scenario_rows` 호출 후 신규 응답(R8). 행 인덱스 산식은 R17.
...
- **proto·worker·migration·store 스키마 0-diff** — 데이터 흐름이 컨트롤러 in-process에서 끝난다(ADR-0026 유지). run 경로(`spawn_run`·`binding.rs`의 기존 함수 시그니처)도 무변경(`apply_mappings`·`collect_var_names`는 재사용만).
...
| R3, R4, R5, R6, R10, R17 | engine/controller 단위 테스트(시드·jar·예산·계속 진행·행 산식) | |
| R9 | controller 단위 8케이스 + 라이브 curl 대표 422 | ✅ |
| R11–R15 | RTL/vitest + 라이브 Playwright | ✅ |
| **US1–US4** | **도그푸딩 체크리스트(라이브 필수)**: 로깅 echo 서버(와이어 grep — 주입 행 값이 *실제 전송*됐는지 리포트가 아닌 와이어로 확인)로 ① US1: 17번 행 선택 → 요청 바디에 그 행 값 grep ② US2: 실패 행 섞인 데이터셋 순차 → 행별 ✓/✗·계속 진행·truncated 케이스 ③ US3: 매핑 무설정 자동 주입 + 명시 매핑 재지정 ④ US4: 같은 데이터셋·같은 행으로 실제 run(1 VU) 생성 → echo 서버 와이어와 test-run 와이어 동일값 대조 | ✅ |
