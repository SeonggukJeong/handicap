...
- [ ] **Step 1: 의존성 추가 (워크스페이스 경유 — repo 컨벤션, 리뷰 M1)**

이 repo는 모든 의존성을 root `[workspace.dependencies]`에 두고 crate에서 `<name>.workspace = true`로 참조한다(axum/serde/sqlx/tonic… 전부). 동일 패턴을 따른다.

① root `Cargo.toml`의 `[workspace.dependencies]`에 세 줄 추가:
```toml
croner = "3"
chrono = "0.4"
chrono-tz = "0.10"
```
② `crates/controller/Cargo.toml`의 `[dependencies]`에 세 줄 추가(엔진/워커는 안 씀 — controller만):
```toml
croner.workspace = true
chrono.workspace = true
chrono-tz.workspace = true
```
> 메모(리뷰 M2): `chrono`/`chrono-tz`를 `[dev-dependencies]`에 **중복 추가하지 말 것**. `[dependencies]`는 통합 테스트(`tests/*.rs`)에서도 보이므로 Step 3의 테스트가 그대로 `use chrono`/`chrono_tz`를 쓸 수 있다(default features로 `chrono_tz::Asia::Seoul`·`with_ymd_and_hms` 가용 — 리뷰가 실빌드로 확인).
...
```bash
git add Cargo.toml Cargo.lock crates/controller/Cargo.toml crates/controller/src/lib.rs \
        crates/controller/src/schedule/ crates/controller/tests/scheduler_trigger_test.rs
...
Run: `cargo test -p handicap-controller --test api_test --test data_binding_api_test --test presets_api_test --test datasets_api_test --test run_dispatch_failure_test`
Expected: 전부 PASS. (테스트 파일명이 다르면 `ls crates/controller/tests/`로 확인 — run-create를 다루는 통합 테스트 집합을 기준선으로 잡는다.) 이 집합이 Step 4의 회귀 게이트.

> TDD-guard: 이 task는 `runs.rs`(src) 텍스트를 바꾸는 순수 리팩터다. **`runs.rs`는 이미 인라인 `#[cfg(test)] mod tests`를 갖고 있어**(`:622`, 리뷰 확인) Edit이 자동 통과한다 — keepalive 불필요. 진짜 회귀 게이트는 위 기존 통합 테스트가 GREEN인 것.
...
## 34b 인계 메모 (plan 리뷰 m1/m2)

- **croner는 필드수에 관대**(리뷰 m1, 실빌드 확인): `Cron::from_str`이 5-field뿐 아니라 6-field도 파싱한다 — 즉 `validate_trigger`(cron 파싱 성공만 봄)는 고급 raw 탭의 stray 6-field를 **거부하지 않는다**. spec §5 불변식 "전부 5-field로 통일"을 강제하려면 34b의 `validate_trigger`(또는 API 검증)에서 필드수==5 추가 체크가 필요한지 결정. (UI 프리셋은 항상 5-field라 정상 경로는 무영향.)
- **`validate_trigger`는 TZ-free**(리뷰 m2): 34b 루프의 due-계산은 TZ-aware(`next_fire_after(_, _, tz)`)지만 `validate_trigger(t, now_ms)`엔 TZ 인자가 없다(once=epoch ms 비교, cron=파싱만 — 둘 다 TZ 무관). 34b API 핸들러가 `validate_trigger`에 TZ를 넘기려 하지 말 것.
