...
- `:35` `{error && <p className="text-red-600">{ko.common.failedToLoad((error as Error).message)}</p>}` → `{error && <Callout variant="error" className="mb-3">{ko.common.failedToLoad((error as Error).message)}</Callout>}` — **`mb-3`는 의도적 spacing-unify**(원본 `:35`엔 마진 없으나 인접 형제 clone 오류 `:37`가 `mb-3`라 통일; `:138`/`:82`는 컴포넌트 전체를 대체하는 early-return이라 마진 불요와 대조). 변환 박스가 page-flow 인라인이면 인접 형제 마진에 맞춤.
...
<!-- spec-plan-reviewer clean APPROVE: spec 2R (2026-06-28), plan 1R (2026-06-28). -->
