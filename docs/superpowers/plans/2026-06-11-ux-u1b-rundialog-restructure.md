...
```ts
    ...
    const reasons: string[] = [
      ...
      // datasetGone이면 columns=[]라 모든 column 행이 stale로도 잡혀 사유가 행 수만큼
      // 중복된다 — 삭제 사유 1줄만 남기도록 stale은 !datasetGone으로 게이트.
      ...(datasetGone
...
```
  ...
  (boolean 판정 의미는 무변경: `uncovered.length===0 && staleCols.length===0 && !datasetGone` — datasetGone이면 reasons가 비지 않으므로 동일.)
...
1. `sloOpen`/`pacingOpen`/`advancedOpen` 3개 토글 state → **`advancedOpen` 1개로 통합** (single-level, spec §6.1):
   ...
   - 프리셋 load(L175·L180의 `setSloOpen(true)`/`setPacingOpen(true)`)는 `setAdvancedOpen(true)` 한 곳으로 — 조건은 **criteria/think/타임아웃≠30/루프캡≠256만** (`prof.measure_phases`는 제외: `loadPreset`은 현재 `measurePhases` state를 시드하지 않는 기존 갭이 있어, 그걸 조건에 넣으면 "펼쳤는데 체크박스 꺼짐" 불일치가 생긴다 — 갭 수정은 U1b 범위 밖, 주석으로 명시).
