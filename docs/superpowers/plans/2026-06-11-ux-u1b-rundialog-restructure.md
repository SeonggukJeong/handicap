...
- **[C] 혼합 2개**: L443(rename — 셀렉터가 `load preset`·`이름 변경`뿐이라 **갱신 0건**, 사실상 A), L1085(http timeout) — payload 줄 무수정, 라벨 줄 갱신.
...
| `Cancel` | `취소` | **`ui/src/pages/__tests__/ScenarioRunsPage.test.tsx` L129·L144**의 `getByRole("button",{name:"Cancel"})` → `"취소"` (Task 4에서) |
| `Closed-loop (VU)` (라디오) | `사용자 수 기준 (closed-loop)` | `/closed-loop/i` → `/사용자 수 기준/` |
| `Open-loop (rate)` (라디오) | `요청 속도 기준 (open-loop)` | `/open-loop/i` → `/요청 속도 기준/` |
| `VUs` | `동시 사용자(VU)` | `"VUs"` → `/동시 사용자/` — **RunDialog.test 외에 `ui/src/pages/__tests__/ScenarioRunsPage.test.tsx`의 `getByLabelText("VUs")` 9곳(L91,L121,L128,L130,L132,L143,L145,L149,L166 — 다이얼로그 열림/닫힘 판정 셀렉터)도 Task 2에서 함께 갱신** |
...
- [ ] **Step 2: RED 확인** — Run: `cd ui && pnpm test ko` / Expected: 신규 1개 FAIL(`runDialog` 프로퍼티 없음 — esbuild 경로라 TS 에러가 아니라 런타임 undefined 접근 실패), 기존 2개 PASS
...
- [ ] **Step 4: GREEN** — Run: `cd ui && pnpm test ko` / Expected: PASS (3 tests)
...
- Modify(Test): `ui/src/pages/__tests__/ScenarioRunsPage.test.tsx` (`getByLabelText("VUs")` 9곳 → `/동시 사용자/`)
- Modify: `ui/src/components/LoadModelFields.tsx`

**범위**: 매핑 표 중 LoadModelFields 소유분(라디오 2종·VUs·Duration·Ramp-up·Target RPS·Max in-flight + 영어 에러 3종) + chips + **HelpTip 6곳**(closed/open 라디오 2 + VU·점진 시작·목표 RPS·동시 요청 상한). **불변**: stage aria 3종, `부하 모델`/`프로파일`/`고정`/`곡선`/`부하 모양`/`+ 단계 추가`/인라인 헬퍼 3종/stages role=alert 에러. **chips는 spec §6.2의 4번째 "직접 입력" chip을 의도적으로 생략** — 어떤 chip과도 불일치한 입력값 = 암묵적 직접 입력 상태(aria-pressed 전부 false)로 같은 의미를 더 단순하게 표현(의도적 deviation, 최종 리뷰어 참고).
...
- [ ] **Step 3: 구현** — `LoadModelFields.tsx`:
  ...
  **⚠ HelpTip 배치 제약 (HTML 유효성)**: HelpTip은 `<button>`이라 **`<label>` 안에 넣으면 안 된다**(label 내 interactive 요소는 불법이고, 클릭이 라디오/입력 활성화로 전달됨). 패턴:
  ...
  3. chips 삽입 — **이미 `loadModel === "closed"` ternary 분기 *내부*인 그리드 div(L115) 바로 위**에 삽입하므로 별도 `{loadModel === "closed" && …}` 가드 불필요(plain div):
...
- [ ] **Step 5: RunDialog.test.tsx + ScenarioRunsPage.test.tsx 드라이버 갱신 (이 task가 바꾼 라벨만)** — `ScenarioRunsPage.test.tsx`의 `getByLabelText("VUs")` 9곳(L91,L121,L128,L130,L132,L143,L145,L149,L166)을 `/동시 사용자/`로. RunDialog.test는 매핑 표대로: `"VUs"`(L305,L539,L865,L873 등)→`/동시 사용자/`, `/Duration/`·`"Duration (s)"`(L137,L306,L540,L875,L1006)→`/테스트 시간/`, `/Ramp-up/`(L82,L141,L307,L541,L866,L874,L1165)→`/점진 시작/`, `/target rps/i`(L800,L809,L846,L849,L1010)→`/목표 RPS/i`, `/max in.?flight/i`·`"Max in-flight"`(L801,L822,L854,L901,L1013)→`/동시 요청 상한/`, `/closed-loop/i`(L834,L864,L1053)→`/사용자 수 기준/`, `/open-loop/i`(L799,L807,L820,L835,L843,L872,L894,L921,L933,L962,L972,L1004,L1050,L1055,L1064)→`/요청 속도 기준/`, 에러 문구 L147→`/점진 시작은 테스트 시간 이하/`, L812→`/목표 RPS는 1 ~ 1,000,000 사이/`, L825→`/동시 요청 상한은 1 ~ 10,000 사이/`. **payload 단언 줄(분류 A)은 무수정.**

- [ ] **Step 6: 전체 GREEN** — Run: `cd ui && pnpm test RunDialog && pnpm test LoadModelFields && pnpm test ScenarioRunsPage` / Expected: 53 + 11 + 전체 PASS

- [ ] **Step 7: 커밋**

```bash
git add ui/src/components/LoadModelFields.tsx ui/src/components/__tests__/LoadModelFields.test.tsx ui/src/components/__tests__/RunDialog.test.tsx ui/src/pages/__tests__/ScenarioRunsPage.test.tsx
...
```tsx
  it("미커버 변수가 있으면 reasons에 변수명 사유가 들어간다", async () => {
    ...
    // 복제 대상 L203 테스트의 fixture(makeScenarioWithMissing)는 {{missing}} 변수 사용
...
```
  ...
  (datasetGone이면 `columns=[]`라 모든 column 행이 stale로도 잡혀 사유가 중복되므로 삭제 사유 1줄로 배타 처리. boolean 판정 의미는 무변경 — datasetGone이면 reasons가 비지 않아 기존 `uncoveredCount===0 && noStaleColumns && !datasetGone`과 동치.)
...
- Modify(Test): `ui/src/pages/__tests__/ScenarioRunsPage.test.tsx` (`"Cancel"` 버튼 셀렉터 2곳 L129·L144 → `"취소"`)
- Modify: `ui/src/components/RunDialog.tsx`
- Modify: `ui/src/components/CriteriaFields.tsx` (FIELDS label 11종만 — 매핑 표)

**구조 변경 요지** (아래 item 1–6이 권위 — item 1의 토글 state 통합과 item 3의 `bindingBlock` 교체 **외의** state·헬퍼·프리셋 로직·buildProfile은 전부 무변경):
...
1. `sloOpen`/`pacingOpen`/`advancedOpen` 3개 토글 state → **`advancedOpen` 1개로 통합** (single-level, spec §6.1):
   ...
   - 프리셋 load(L175·L180의 `setSloOpen(true)`/`setPacingOpen(true)`)는 `setAdvancedOpen(true)` 한 곳으로 — 조건은 **criteria/think/타임아웃≠30/(`hasLoop &&` 루프캡≠256)만** (초기값 식과 동일하게 루프캡엔 `hasLoop` 게이트 — no-loop 프리셋은 cap이 0으로 저장되는데 입력 자체가 안 보이므로 헛펼침 방지) (`prof.measure_phases`는 제외: `loadPreset`은 현재 `measurePhases` state를 시드하지 않는 기존 갭이 있어, 그걸 조건에 넣으면 "펼쳤는데 체크박스 꺼짐" 불일치가 생긴다 — 갭 수정은 U1b 범위 밖, 주석으로 명시).
...
6. **금지**: state 초기화 식·`loadPreset`·`buildProfile`·`currentInput`·프리셋 CRUD·`canSubmit`의 불리언 구조(이름 교체 제외)·`env` 계산·`setCriteria` cross-field — 전부 무변경 (단 item 1이 명시한 `advancedOpen` 통합 초기값·펼침 호출 변경은 예외).

- [ ] **Step 1: RunDialog.test.tsx + ScenarioRunsPage.test.tsx 갱신** — `ScenarioRunsPage.test.tsx` L129·L144의 `getByRole("button",{name:"Cancel"})` → `"취소"`. RunDialog.test는 매핑 표의 나머지(Task 2에서 안 한 것): `/^Run$/`→`/^실행$/`(전 테스트), `/HTTP timeout/i`→`/HTTP 타임아웃/`(L309,L1063,L1065,L1067,L1076,L1103), `/loop breakdown cap/i`→`/루프 집계 상한/`(L187,L214,L234,L308), think 3종(L677–679,L732–744), criteria 라벨(L566–567,L586–590,L623–624,L1142–1144,L1170–1172), 에러 L1082→`/HTTP 타임아웃은 1 ~ 600초 사이/`, 토글: `getByRole("button",{name:/SLO 기준/})`(L565,L585,L596,L619,L1141,L1169)→`/판정·고급/`, `/Pacing/`(L676,L730)→`/판정·고급/`. **disclosure 테스트 재작성 2개**: L593(기본 접힘+토글 펼침)은 `/판정·고급/` 버튼 + `/최대 p95/` 부재→존재 + **타임아웃 입력도 접힘 시 부재**(`queryByLabelText(/HTTP 타임아웃/)` null) 단언으로 확장; L604(seed 자동 펼침)는 동일 구조에 `/판정·고급/` aria-expanded="true". **타임아웃·캡이 접힘으로 들어갔으므로 그 입력을 만지는 테스트(L187,L214,L1076,L1103,L1063-구간 + L232 테스트의 부재 단언 L234)는 먼저 `await user.click(screen.getByRole("button",{name:/판정·고급/}))`로 펼친다**(ui/CLAUDE.md collapsible 함정 — 접힌 입력은 DOM에 없음; L234는 펼친 뒤에도 부재여야 의미 보존 — 접힘 상태 부재는 vacuous). **예외: L303 테스트(L309 포함)는 fixture가 `http_timeout_seconds:120`·`loop_breakdown_cap:128`이라 통합 초기값 식이 자동 펼침(true) — 클릭하면 오히려 접히므로 클릭 금지, 셀렉터만 교체.** 마지막으로 **payload 기본값 불변식 테스트 1개 추가**:

```tsx
  it("payload byte-identical: 기본값 제출 payload가 재구성 전과 동일하다", async () => {
    const user = userEvent.setup();
    // 기존 payload 테스트 패턴(L150–181 동형): run POST 응답 mock 필수 — 없으면
    // request()가 throw하고, click 직후 동기 접근은 race(POST 기록은 microtask 뒤).
    fetchMock.mockImplementation((url, init) =>
      jsonResponse({ id: "R1", status: "pending" }),
    ); // ← 파일의 기존 fetch stub/jsonResponse 헬퍼 이디엄을 그대로 따를 것
    renderDialog();
    await user.click(screen.getByRole("button", { name: /^실행$/ }));
    await waitFor(() => {
      expect(runPostCall()).toBeDefined(); // 기존 L94–101 캡처 패턴의 인라인/헬퍼화
    });
    const body = JSON.parse(runPostCall()!.body as string);
    expect(body).toEqual({
      scenario_id: "S1",
      profile: {
        vus: 2,
...
```
  ...
  (mock·캡처는 이 파일의 기존 이디엄(L64 테스트의 fetch stub + L94–101 POST 캡처)을 정확히 따른다 — 위 `fetchMock`/`jsonResponse`/`runPostCall`은 그 이디엄의 자리표시 이름이며 실제 파일의 기존 헬퍼·패턴명을 사용할 것. **`toEqual`은 깊은 완전 일치** — 재구성이 필드를 추가/누락하면 즉시 RED. 기대 객체는 RED 단계에서 현 master의 실제 payload 캡처로 확정 — buildProfile이 undefined 필드를 생략하므로 JSON.stringify 결과 기준.)

- [ ] **Step 2: RED 확인** — Run: `cd ui && pnpm test RunDialog` / Expected: 갱신·신규 테스트 다수 FAIL(구 구조 렌더 중)

- [ ] **Step 3: 구현** — 위 구조 변경 요지 1–6 적용.

- [ ] **Step 4: GREEN** — Run: `cd ui && pnpm test RunDialog && pnpm test ScenarioRunsPage` / Expected: 54 PASS (53 + 불변식 1) + ScenarioRunsPage 전체 PASS

- [ ] **Step 5: 커밋**

```bash
git add ui/src/components/RunDialog.tsx ui/src/components/CriteriaFields.tsx ui/src/components/__tests__/RunDialog.test.tsx ui/src/pages/__tests__/ScenarioRunsPage.test.tsx
