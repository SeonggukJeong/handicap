...
Expected: 신규 5개 중 compact 케이스들 FAIL (`compact` prop 미존재 → `py-0.5` 부재), 기존 케이스 PASS.
...
Expected: 신규 5개 FAIL(card variant 미존재·기본 variant hint가 버튼 안), 기존 2개 PASS. (`pnpm test Section`은 `Section.test.tsx`만이 아니라 이름 매치 파일들을 돌릴 수 있음 — `PageSection.test.tsx`도 함께 돌면 그것도 PASS여야 정상.)
...
2. **완성도 게이트 직접 재실행 (spec §6.3)**: `grep -rn "function InspectorSection\|<InspectorSection" ui/src` → 0 / RunListControls에서 `py-0\.5` raw `<select`/`<input` 잔존이 동결 명시(투명 select)만인지 / R7 동결 사이트 라인 무변경 diff.
