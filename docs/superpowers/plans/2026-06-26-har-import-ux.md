...
> **tdd-guard 순서(F3):** 이 태스크는 **테스트 파일을 가장 먼저** 편집한다(아래 Step 1). `ko.ts`·페이지 src 편집은 그 RED diff가 생긴 *뒤* Step 3에서. ko 키 추가도 production src라 테스트보다 먼저 만지면 `tdd-guard`가 막는다.

- [ ] **Step 1: 실패 테스트 작성** — `ScenarioImportPage.test.tsx`에 중복 포함 HAR fixture + describe 추가.
...
Expected: FAIL — `ko.import.selectAll` 등 키·버튼·배지 미정의 → RED.

- [ ] **Step 3: ko 키 + 페이지 구현** (RED diff 생긴 뒤 src 편집 — tdd-guard 통과)

먼저 `ui/src/i18n/ko.ts`의 `import: { … }` 블록에 키 추가(기존 키 뒤, `toEditor` 다음):
...
이어 `ScenarioImportPage.tsx` 수정:

(a) import 보강:
```ts
import { type Har, type PreviewEntry, distinctHosts, duplicateIndices, entryHost, isStaticAsset } from "../import/filters";
```
...
> **tdd-guard 순서(F3):** 테스트 파일(아래 Step 1)을 가장 먼저 편집한다. `ko.ts`·페이지 src는 그 RED diff가 생긴 *뒤* Step 3에서.

- [ ] **Step 1: 테스트 인프라 + 실패 테스트** — `ScenarioImportPage.test.tsx`.
...
(b) fetch stub helper(파일 상단, afterEach 부근) + 기존 `afterEach`에 `vi.unstubAllGlobals()` 추가(opt1 — `vi.stubGlobal("fetch")` 누수 방지, 레포 관행):
```ts
...
먼저 `ui/src/i18n/ko.ts`의 `import: { … }` 블록에 키 추가(Task 2 키 뒤):
...
(b) **상태**(`useState`/`useCreateEnvironment`) — 기존 `useState`들 바로 아래:
```ts
...
(b2) **파생 메모** — `previewEntries` 메모 **뒤**, `yaml` 메모 **앞**에 둔다(F4: `useMemo` 콜백·deps가 그 줄에서 동기 실행되므로 `previewEntries`(const) *선언 뒤*여야 TDZ ReferenceError를 피한다):
```ts
