...
주의: `InsightCompareMatrix.test.tsx`엔 현재 `ko` import가 없다 — `import { ko } from "../../../i18n/ko";` 추가(compare/__tests__ 깊이, 리뷰 P5).
...
// AFTER (prettier printWidth 100 관례상 멀티라인)
    <PageSection
...
```
(**금지**: 같은 파일의 다이얼로그 카드·드롭다운·busy/idle 텍스트·확인/적용 버튼(`bg-accent-600`)·red 파괴 버튼은 동결 — R7.)
...
- [ ] **B. §6 규칙 grep 직접 재실행** (대상군 = `ui/src/components/report ui/src/components/compare ui/src/pages/{RunDetailPage,ScenarioComparePage,ScenarioRunsPage,ScenarioListPage,WorkerDashboardPage}.tsx`; ①–③은 **`--exclude-dir=__tests__` 필수** — Task 3이 Summary.test.tsx에 넣는 lockstep 단언 문자열이 ①에 오탐돼 false-fail, 리뷰 P1):
  1. `grep -rn --exclude-dir=__tests__ 'text-lg font-semibold mb-2' <대상군>` → 잔존 = `VerdictPanel.tsx` + `RunDetailPage.tsx`(metricWindows bare h3) **정확히 2건**
  2. `grep -rn --exclude-dir=__tests__ '<h4 className="text-sm font-semibold text-slate-700' <대상군>` → 잔존 = `ActiveVuChart.tsx` 멀티워커 분기 **1건** (h4-앵커 필수 — bare 서브스트링은 ScenarioSnapshot 버튼 오탐)
  3. `grep -rn --exclude-dir=__tests__ 'rounded px-1.5 py-0.5 text-xs font-medium' <대상군>` → **0건**
  4. `grep -rn '핵심 인사이트' ui/src --include='*.tsx'` → **0건** (ko.ts만 잔존)
  5. **R12 diff 스윕(리뷰 P2)**: 이 브랜치 diff의 **production 파일**(`ui/src/**` non-test)에 ① 신규 하드코딩 한글 리터럴 0(python sweep — `'"[가-힣]'` grep의 비한글-선두 누락 함정 회피; Task 3 `insightsTitle`은 ko.ts로의 *이주*라 무위반, 신규 *테스트*의 한글 리터럴은 기존 관례상 제외) ② 신규 `blue-*`/`indigo-*` 컨트롤 색 리터럴 0(`git diff master..HEAD -- 'ui/src/**' | grep '^+' | grep -E 'blue-|indigo-'` → 0).
