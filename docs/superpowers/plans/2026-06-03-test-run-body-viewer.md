...
- Modify: `crates/engine/CLAUDE.md:34` (stale "16 KiB" 표기 → "1 MiB")
...
같은 task에서 도메인 CLAUDE.md의 stale 표기도 갱신(루트 CLAUDE.md "새 함정을 배우면" 컨벤션). `crates/engine/CLAUDE.md:34`:
```
응답 body는 `MAX_TRACE_BODY_BYTES`(16 KiB)에서 절단
```
→
```
응답 body는 `MAX_TRACE_BODY_BYTES`(1 MiB)에서 절단
```
(그 줄의 `(16 KiB)`만 `(1 MiB)`로 — 나머지 U+FFFD/display 문구는 그대로.)
...
```bash
git add crates/engine/src/executor.rs crates/engine/CLAUDE.md
