...
describe("safeDecodeUrl — 멱등 (R5)", () => {
  it("모든 대표 케이스에서 재적용해도 불변", () => {
    const cases = [
      "https://a.com/%EA%B2%80%EC%83%89?q=%ED%95%9C%20%EA%B8%80",
      "https://a.com/api?redirect=https%3A%2F%2Fb.com%2Fp%3Fx%3D1",
      "https://a.com/q?name=%EA%B9%80%26%EC%9D%B4",
      "https://a.com/q?bad=%EA%B9%80%FF&trunc=%2",
      "${BASE_URL}/my?tab=%ED%99%88",
    ];
...
Expected: 신규 describe 6케이스 FAIL(인코딩 원문이 그대로 나옴), 기존 케이스 전부 PASS.
...
- Task 2 Step 1의 `s.request.headers?.Referer` — `RequestModel.headers`는 `.default({})` 누출로 optional일 수 있어 `?.` 사용(tsc -b 안전).
