...
`ui/src/components/scenario/__tests__/TestRunSection.test.tsx` — ① RTL import에 `within` 추가(`import { act, render, screen, within } from "@testing-library/react";`), ② 기존 `describe("TestRunSection flow chip strip (spec R1/R6)")` 블록 안에 케이스 추가:

```tsx
  it("칩 스트립 overflow 시 펼치기 토글 노출 — expandable 배선 (editor-wide-view-overflow US1)", () => {
...
(파일의 기존 `afterEach(vi.clearAllMocks)`는 spy 원복을 안 하므로 케이스 말미 `vi.restoreAllMocks()`로 prototype getter를 반드시 되돌린다 — 다음 케이스 오염 방지.)
...
`ui/src/components/scenario/__tests__/EditorShell.test.tsx` — ① vitest import에 `vi`가 없으면 추가, ② 기존 `describe("스텝 넓게 보기 토글 (R5/R10)")` 블록 안에 케이스 추가:

```tsx
    it("wide 칩 스트립엔 펼치기 토글 부재 — expandable 미배선 락인 (overflow여도)", async () => {
