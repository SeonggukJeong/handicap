...
- 실행은 `.claude/worktrees/<name>` 워크트리에서 (`EnterWorktree`, `worktree.baseRef: head`). 새 워크트리엔 `ui/node_modules`·`target/`이 없으니 첫 task 전 `cd ui && pnpm install` + **`cargo build -p handicap-worker && cargo build --workspace`로 warm** — 안 하면 첫 커밋의 pre-commit이 cold build라 문서화된 e2e flake(worker ENOENT/SIGKILL/`unix_wait_status(15)`, UI-only 커밋도 발생)가 난다. flake 시 동일 커밋을 warm 상태로 재시도.
...
Expected: 신규 첫 테스트 1개만 FAIL (`Unable to find ... role "button" name "p50 설명"`) — 둘째 신규 테스트는 negative 단언 가드라 RED 단계에서도 PASS가 정상. 기존 5개 PASS.
...
- [ ] **Step 2: 실패 시** — 원인 수정 후 **별도 fix 커밋**(amend 금지), 게이트 재실행.

(이 task는 게이트 확인만 — 통과 시 커밋 없음.)
...
- [ ] 라이브 확인 1회: 워크트리 자체 바이너리(`cargo build -p handicap-worker --bin worker && cargo build -p handicap-controller --bin controller` 후 `./target/debug/controller --db /tmp/u1a.db --ui-dir ui/dist`, 먼저 `just ui-build`)로 띄우고, run이 0건이므로 **시나리오+run 1개 생성**(curl `POST /api/scenarios`+`POST /api/runs` — 루트 CLAUDE.md curl 레시피, 또는 RunDialog) 후 리포트 열어 p95 ⓘ 클릭 → 설명 popover, ESC 닫힘, **우단 카드 p99 ⓘ도 클릭(클리핑 확인)**, 콘솔 에러 0 (인라인 `browser_snapshot`/`browser_evaluate` — `filename` 저장 금지, Playwright cwd 함정)
