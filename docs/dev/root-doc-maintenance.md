...
## 재분배 절차 — manifest 작성 → 목적지 기입 → 소스 축약 → 검증 (순서 고정)

재분배는 아래 5단계를 **이 순서로** 밟는다. 이 순서(이관을 압축보다 먼저)는 의도적이다 — 중간에 끊겨도 이미 목적지에 쓴 내용과 소스 원본이 공존하는 상태이지 지식이 사라진 상태가 아니다.
...
3. **소스 축약** — 소스(root `CLAUDE.md`·`ui/CLAUDE.md` 등 게이트에 넘긴 검사 대상)에서 그 `source_anchor`를 지운다(요약 또는 삭제). 아직 지우지 않았으면:
