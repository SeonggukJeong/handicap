...
- **2026-07-25 (도그푸딩 피드백 2)**: B 점검 어휘 `만들다` → **`정의`**. `참조하는 변수를 모두 만듭니다`는
  "무엇이 무엇을 만드는지"가 없어 읽는 사람이 주어를 찾게 된다(실제로는 아무것도 생성되지 않고,
  참조하는 변수에 **출처**가 있다는 뜻 — `variables:` 선언 또는 스텝 `extract`). 게다가 앱은 이미
  같은 개념을 `정의`로 부른다: 변수 패널 배지 `정의안됨`(`ko.editor.variableUndefined`)·title
  `{name} — 정의되지 않은 변수`·이 파일 섹션명 `미정의 변수`·내부 id `undefined_vars`. B가 혼자 다른
  어휘를 쓰면서 **그 패널로 링크**하고 있었다.
  → `checkBFailTitle`=`정의되지 않은 변수 N개를 참조합니다`, `checkBPass`=`참조하는 변수가 모두 정의돼 있습니다`.
  **겹침 주의(무해, 기록)**: 구절 `정의되지 않은 변수`가 패널 `title`(전체 문자열 `{name} — …`, 테스트는
  `getByTitle` 정확매치)과 이 모달 **텍스트** 양쪽에 등장한다 — 쿼리 종류가 달라 현재 충돌 없으나,
  Playwright `getByLabel`(기본 부분일치)로 이 구절을 잡으면 두 표면이 함께 걸릴 수 있다.

- **2026-07-25 (도그푸딩 피드백)**: `naLabel`("해당 항목 없음") **폐기** → 점검별 `checkANa`/`checkBNa`/`checkCNa`.
  이유: 접힘 목록의 통과 줄들은 내용으로 자기를 식별하는데(`모든 스텝에 응답 검증이 있습니다`) na 줄만
  맥락 없는 공통 문구여서 **처음 보는 사용자가 무엇이 해당 없는지 알 수 없었다**. 실사용에서 보이는 na는
  사실상 C 하나(추출이 없는 시나리오)다. 부수 효과로 `통과한 점검 2개`가 3줄로 펼쳐지는 불일치(D7이
  `passed`만 세는 의도된 결과)도 자기설명적으로 해소된다.
