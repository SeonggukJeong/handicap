...
- **방향만 확정, 코드 변경 0**(이 ADR은 기록). 구현은 roadmap 후보로 **미착수 — 문서로만 남긴다.**
- **착수할 경우 첫 슬라이스 범위 = 옵션 A(단일 self-contained `.exe`)만.** 옵션 B(Tauri 래퍼)는
  옵션 A가 동작한 뒤 "네이티브 앱 경험이 필요해지면" 비로소 검토하는 **별도 후속 결정**이며 첫
  착수에 포함하지 않는다(사용자 지시 2026-06-16: "일단 하더라도 exe 단일 파일부터").
