...
## Slice 9c — 상호 1레벨 중첩 (2026-05-31 추가)

9c(UI-only + 엔진 가드 테스트 1개)가 구현·머지됐다.

**UI 게이트 — 두 계층 Zod 모델(two-tier):**
분기(`then`/`elif[].then`/`else`)가 http-only인 `NestedLoopStepModel`/`NestedIfStepModel`/`NestedElifBranchModel` (1레벨 깊이 한정) + 최상위 `LoopStepModel`/`IfStepModel`(본문이 `http | 상대 Nested*` discriminatedUnion)로 게이트를 구성한다. 이 구조는 **타입 자체가 비허용 경로를 차단한다(by construction)**. `z.lazy` 자기참조를 쓰지 않은 이유: 자기참조 `StepModel`은 동일 타입 무제한 중첩(loop-in-loop, if-in-if)을 허용해 §5 "상호 1레벨" 불변식을 깨고, 중첩 `.default()` input 누출(`tsc` 함정 — `ui/CLAUDE.md` 참고)도 유발한다.

**엔진:** 프로덕션 코드 무변경. 엔진은 이미 `Vec<Step>` 자유 재귀를 허용하므로 loop-in-if 경로를 커버하는 특성화/가드 테스트(`loop_in_if_then_repeats_body`, `crates/engine/tests/if_node.rs`) 1개 추가로 9c 완료.

**컨트롤러·proto·워커·메트릭·runs 테이블 무변경.** 분기별 메트릭 breakdown은 **9d**.

## Slice 9d — 분기별 메트릭 breakdown (2026-05-31 추가)

9d가 구현·머지됐다. Slice 7-1의 loop_index 카운터 파이프라인과 동형이지만 세 가지 설계 차이가 있다.

**파이프라인 개요:**
엔진 `Aggregator::record_branch(step_id, branch)` → `MetricFlush.branch_stats`(세 번째 드레인 벡터) → proto `BranchStat{step_id,branch,count}` / `MetricBatch.branch_stats = 5` → 워커 포워드 → controller `run_if_metrics` 테이블(migration **0006**, `CREATE TABLE IF NOT EXISTS` 멱등, PK `(run_id,step_id,branch)`) UPSERT-accumulate → `ReportJson.if_breakdown: Vec<IfBreakdown>` → UI `BranchStatsTable`("Branch decisions" 섹션).

**엔진 기록 시점:** `Step::If` arm이 분기를 실행한 직후 레이블(`"then"` / `"elif_0"` / `"elif_1"` / … / `"else"` / `"none"`)로 `record_branch`를 호출한다. 레이블과 실제 실행 분기는 항상 일치한다. **`none`** = 조건이 false이고 elif도 없거나 모두 false이며 else가 비어있거나 absent — http leaf 없이 nothing을 실행한 결정을 위한 전용 카운터.

**Slice 7-1과의 세 가지 의도된 차이:**
1. **cap 없음 / overflow sentinel 없음**: `if` 1개당 브랜치 수는 유한(`then + elif_n + else + none`)이라 cap이 불필요. `loop_breakdown_cap` proto 필드·sentinel 로직 무관.
2. **`error_count` 없음**: 분기 결정은 HTTP 요청이 아니라 카운터 — `error_count` 개념 자체가 적용 안 됨.
3. **`ReportStep`이 아닌 최상위 `ReportJson.if_breakdown`**: `if` 노드 id는 http-leaf 메트릭(= `report.steps`) 행에 나타나지 않고, `none` 버킷은 http leaf 자체가 없어 `ReportStep`에 붙일 수 없다. 명세 §7의 리터럴 `ReportStep.branch_breakdown`은 구현 불가 — 별도 최상위 배열로 대체. UI는 `StepStatsTable` 뒤에 독립된 `BranchStatsTable`로 렌더.

**UI 정렬 보정:** SQL은 `branch` TEXT 사전순으로 반환(lexicographic)하므로 UI 컴포넌트(`BranchStatsTable`)가 `branchRank`로 `then < elif_n < else < none` 순서로 재정렬. `none` 레이블은 `(미매치)`로 표시. `ifMeta`는 `findStepById`(9c 추가)로 if 노드 표시명을 resolve.

**e2e:** `if_branch_report_e2e_smoke`(controller `e2e_test.rs`)가 워커 subprocess → gRPC `branch_stats` → `run_if_metrics` → `/report.if_breakdown` then-count > 0을 검증.

## 명시적 연기 (Out of scope, future slices)

- **if-in-if**, **loop-in-loop**, 더 깊은 자유 중첩 GUI.
- 조건식 문자열 DSL · 정규식 플래그(대소문자 무시 등).
- 분기별 **레이턴시** breakdown(9d는 counts-only, 7-1과 동일 한도 — 향후 A4 LoadRunner급 리포트에서 검토).
