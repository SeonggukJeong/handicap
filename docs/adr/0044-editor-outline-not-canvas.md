...
- 상태: 채택됨 (2026-06-29) · 연기 항목 절 단위 해소 — 컨테이너 경계 넘는 드래그/re-parent(§결과 3번째 항목)는 2026-07-02 `editor-reparent-dnd`로 해소(`docs/build-log.md:207`)
...
- **그룹내 드래그 재정렬(`dnd-kit`)**: 형제 그룹마다 `SortableContext`, `PointerSensor`+`KeyboardSensor`. `onDragEnd` → 순수 `resolveDragEnd(steps, activeId, overId)` → 기존 `moveStep(stepId, toIndex)`. **그룹 경계 넘는 re-parent는 no-op**(슬라이스 3 연기 — → 2026-07-02 `editor-reparent-dnd`로 해소, `docs/build-log.md:207`). 신규 모델 변경 0 — `moveStep`은 이미 형제-그룹 재정렬을 했고 드래그 어피던스만 없었다.
...
- **트레이드오프**: 자유형 공간 배치를 잃는다 — 시나리오는 순차/중첩 트리이지 그래프가 아니므로 수용 가능. **컨테이너 경계 넘는 드래그/re-parent(슬라이스 3)**(→ 2026-07-02 `editor-reparent-dnd`로 해소, `docs/build-log.md:207`), **하단 흐름 다이어그램 + 테스트 결과 색상(슬라이스 2)**, YAML file-I/O(fast-follow — share→paste는 모달로 충족)는 연기.
