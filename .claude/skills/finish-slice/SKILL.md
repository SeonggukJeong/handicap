...
- [ ] **보안 표면 게이트(path-gated — blanket 아님)**: 슬라이스 diff가 보안 민감 표면(요청 실행·템플릿/캐스트·env/데이터셋 바인딩·업로드 파싱·trace/body 뷰어 = SSRF·시크릿 누출·템플릿 인젝션이 사는 곳)을 건드리면 `security-reviewer` APPROVE 필수. 기계적으로 판정:
  ...
  ```
  매치 있음 → `security-reviewer` 실행. **종료 규칙(무한 루프 금지 — "APPROVE까지" 아님)**: ① **Critical/High(SSRF·시크릿 누출·인젝션 *실재*)만 머지 차단** — fix 후 *그 fix만* focused 재검토 **1회**로 끝. ② Low/Info·하드닝 제안은 **build-log에 기록하고 머지 진행**(재검토 루프 안 돎). ③ 각 finding은 `receiving-code-review`로 타당성부터 판정 — 틀림/범위밖이면 근거 1줄로 **기각**(맹종 금지). ④ **2라운드 안에 Critical/High가 미수렴**(reviewer가 같은 류를 계속 제기하거나 새 Critical/High를 끝없이 생성)이면 **루프 중단하고 사용자에게 disagreement 요약 escalate**. 매치 없음 → **build-log에 "보안 표면 무관(N/A)" 한 줄로 명시 스킵**(UI 폴리시·리포트·docs 슬라이스는 대부분 여기). (`scenario.rs` URL-검증 변경 등 set 밖이라도 요청 구성에 닿으면 판단으로 추가.) 이 게이트가 `security-reviewer`의 트리거 — "쓸지 기억"이 아니라 diff가 결정.
