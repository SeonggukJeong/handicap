...
**US-anchored 척추 (user-path 슬라이스 필수)**: 검증 체크리스트는 spec의 `사용자 스토리 (US)` 블록을 척추로 구성 — 행마다 `US | 절차(클릭·curl·와이어) | 통과 신호`. "동작함"(200/렌더)이 아니라 **사용자 작업 재현**을 증명한다. 집계 리포트가 못 보는 것(실제 전송 헤더/폼, DOM 상태)은 와이어/DOM 실측으로(editor-dataset-testrun US1–4 선례). `US: N/A`·재현-블록 슬라이스는 해당 없음. 규약: `docs/dev/user-story-spine.md`.
