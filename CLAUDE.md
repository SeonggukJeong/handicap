...
- **Chrome Safe Browsing은 오프라인에서 blob URL 다운로드를 차단**: `<a href="blob:..." download>` 는 OS 다운로드 매니저를 거치는데, Chrome의 "Enhanced Safe Browsing" 또는 일부 "Standard" 설정도 다운로드마다 Google에 phone home해 검증한다. 사내망/에어갭(ADR-0001)에선 "인터넷 연결 없음"으로 다운로드 실패. 대안: **`window.showSaveFilePicker`** (File System Access API, Chrome 86+ / Edge 86+) 는 다운로드 매니저를 우회하고 사용자가 선택한 파일 시스템 위치에 직접 쓴다 → Safe Browsing 무관. Firefox/Safari는 미지원이라 blob URL fallback 유지 (해당 브라우저는 오프라인 Safe Browsing 차단 없음). 패턴: `if (typeof window.showSaveFilePicker === "function") { ... } else { /* blob URL anchor click */ }`.
