...
**상태: MVP 1단계(슬라이스 1–9) 완료 + post-MVP1 영역 A(프리셋·멀티워커 fan-out·Parallel·리포트 export/insights)·B(환경·SLO criteria·리포트 깊이)·C(에디터 test-run)·D(부하모델·페이싱: 타임아웃/think-time/open-loop/stages/VU 곡선) + Run 스케줄러 + 영역 U(UX, ADR-0035) + 후속 다수(LAN 분산 워커 L1–L7·풀 운영 견고성/제어상태 영속화·Tauri 데스크톱[in-process, ADR-0042]·단일 self-contained exe[ADR-0039]·HAR→시나리오 가져오기·다중 데이터셋 바인딩·운영 상한 관리자·스텝 템플릿 관리·ko.common 한국어화·게이트-에러 한국어 매핑·디자인 시스템[ADR-0043]·RunDialog 간단/상세+목업 시각 충실도 재구성·비교 뷰 깊이/XLSX Δ 조건부 서식·트랜잭션 시간 분해·Run 라이브니스 G1/G3+stall 배지·closed-loop 곡선 fan-out 샤딩·결과화면/스텝 막대 폴리시·에디터 캔버스→아웃라인 재설계·에디터 레이아웃 후속 버그 수정·에디터 드래그 메커니즘 수리·에디터 YAML 모달 가져오기/내보내기·JSON 바디 캐스트 확장·에디터 테스트 흐름 칩 스트립·에디터 경계 드래그/re-parent·시나리오 삭제/이름 라이브 편집·에디터 공간·이름 QoL·Button-accent 색 이주·에디터 변수 도구 A 등 — **전수 목록·구현 결과·함정 출처는 `docs/build-log.md`가 단일 소스**)까지 구현·머지 완료. 최신 = 에디터 UX 폴리시(editor-varpanel-viewport-polish·UI-only·ADR-0044 범위 내[신규 ADR 없음]·crates/proto/migration/model.ts(Zod)/YAML 직렬화/store rename 시그니처/wire 0-diff): VariablesPanel에 4가지 마찰 해소 — #3 사용처 nav 팝오버(portal-fixed body 렌더·overflow-auto 열 미클립·edge-flip·순환 nav 대체·onCloseRef)·#4 에디터 뷰포트 높이 캡(max-h-[calc(100vh-16rem)]+grid-rows-[minmax(0,1fr)]·min-h-[680px] 제거)+열별 내부 스크롤·#5 rename 열린 스텝 draft 3종(헤더/JSON바디/추출) 실시간 재시드(store renameEpoch 성공시만++·Inspector 4 하위컴포넌트 dep)·#6 변수 검색(이름·값·분기표기 부분일치·전용 무매치 문구). SDD 5 task(전 Sonnet 구현·Task3 팝오버 escalate→Opus re-pass[close-path 정확성·anchor-exclusion 확정]·whole-branch handicap-reviewer[Opus] APPROVE + 1 fold-in[검색 필터아웃 시 팝오버 닫기])·보안 N/A(요청/바인딩/파싱 무접촉)·라이브 검증 all-PASS(controller-served dist 8090·getBoundingClientRect/input.value 실측: grid=innerH−256·팝오버 열밖 140px 미클립·rename 3표면 실시간·검색 필터·0 콘솔). 2026-07-04 머지). 완료 슬라이스/기능 상세·함정 출처 → `docs/build-log.md`, 다음 작업(테마별 frontier+추천) → `docs/roadmap-status.md`(현황판·shortlist 대체), 후보 메뉴·연기 항목 상세 → `docs/roadmap.md`, ADR 인덱스 → 아래 [알아둘 결정들](#알아둘-결정들), 결정 전문 → `docs/adr/`. 디자인 → `docs/superpowers/specs/`, 구현 계획 → `docs/superpowers/plans/`. MVP 1단계 spec=`2026-05-27-handicap-mvp1-design.md`(슬라이스 1–6), 후속은 그 §4.5 메뉴에서 각자 새 spec/plan으로.**
