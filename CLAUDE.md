...
**상태: MVP 1단계(슬라이스 1–9) + post-MVP1 영역 A(프리셋·멀티워커 fan-out·Parallel·리포트 export/insights)·B(환경·SLO criteria·리포트 깊이)·C(에디터 test-run)·D(부하모델·페이싱: 타임아웃/think-time/open-loop[+멀티워커 fan-out]/stages/VU 곡선) + Run 스케줄러 + 영역 U(UX, ADR-0035) + 후속 다수(스텝 템플릿·active-VU 시계열·A9 용량/사이징 인사이트·step-level SLO·create-time 사이징 헬퍼 4종[닫힌 VU·열린 슬롯 fixed/curve·worker_count]·mean 지연 프록시[사이징 p50→mean·parity]·A9 포화 원인 귀속[loadgen/sut+지속최대RPS·onset]·CSV/compare 인사이트 표면화·HAR→시나리오 가져오기·다중 데이터셋 바인딩[한 run에 N개 독립]·운영 상한 관리자 화면[op-config 상한 6종 런타임 가변]·단일 self-contained 바이너리[Windows 경량 데스크톱 배포·bundle feature off=byte-identical]·응답기반 extract 작성[test-run 응답 클릭→스텝 extract·UI-only]·스텝 템플릿 관리 페이지+삽입 시 변수 파라미터화[/templates 관리(EnvironmentsPage 미러·생성 없음)+토큰 keep/rename/literal 2-phase 단일 Modal·UI-only]·ko.common 한국어화 일괄 롤업[ui/src 잔존 영어/인라인 한국어 전수→ko.ts 카탈로그·UI-only]·에디터 test-run UX 정리+스킴-없는 URL 검출[헤더 중복 버튼 제거→test-run 단일 섹션 통합·검증배너 스킴-없는 host URL 일반화·UI-only]·게이트-에러 한국어 매핑[Zod discriminator/enum/unrecognized-key/min 클래스→ko.editor.gate* 매핑·실-Zod 3.25.76 드리프트 가드·model.ts 0-diff·UI-only]·Tauri 데스크톱 셸[bundle controller 사이드카 래핑→네이티브 창+Windows 인스톨러·desktop/ 신규(자체 [workspace]로 cargo 워크스페이스 밖)·crates/ui 0-diff]·LAN 분산 워커 L1[상시 워커 풀: `--worker-mode pool`이 유휴 워커 풀에 run push 배정·proto `Register.token` 공유 토큰 인증·LAN 바인드·migration 0/엔진 무변경·ADR-0041]·LAN 분산 워커 L2[워커 대시보드 읽기전용: `/workers` 연결 워커 폴링(hostname/유휴·Busy/capacity)+RunDialog 풀 프리뷰·proto `Register.hostname` additive·`GET /api/pool/workers`·migration 0/엔진 0·ADR-0041 범위]·LAN 분산 워커 L3[과부하 가드: capacity-aware closed-loop 풀 배정·water-fill `capacity_split`(무바인딩=균등 byte-identical)·용량 부족 시 `spawn_run` insert-전 409 `{achievable_vus,requested_vus}`+`?force` 강행·RunDialog 총용량 프리뷰/초과힌트/409 확인다이얼로그·proto/worker/engine/migration 0·ADR-0041 범위]·LAN 분산 워커 L4[open-loop/곡선 과부하 가드: 슬롯=capacity_split·레이트 비례(고정 proportional_split_min1·곡선 proportional_split)·N 레이트상한 min(max_in_flight,rate_peak)·용량부족 409/줄여진행(max_in_flight clamp)/강행·proto/worker/engine/migration 0·ADR-0041 범위]·LAN 분산 워커 L5[closed-loop VU 곡선 과부하 가드: 곡선 peak를 capacity_split로 cap 존중 샤딩·각 워커 vu_stages를 proportional_split로 비례 스케일(R3 peak stage==vu_count)·active-VU 시계열 worker_id 머지(migration 0018 ensure_*+read SUM, N=1 byte-identical)·용량부족 409/곡선 비례 축소(scaleVuStages)/강행(even-split)·예약-시점 unique 빈슬라이스 floor(rows<N reject, 3모드 공통 선재버그fix)+release_pool_reservation(예약 누수 fix)·proto/worker/engine 0·migration 1·ADR-0041 범위]·LAN 분산 워커 L6[하트비트/last-seen/유령 워커 정리: 리퍼가 풀 워커 전원 Ping push·워커 Pong echo·임의 인바운드 last_seen 스탬프·`last_seen>stale` evict(idle 제거/busy worker_disconnected fail-fast, 신규 로직 0)·h2 keepalive 죽은연결 teardown→reconnect→재등록·/workers '마지막 응답' 열+stale 배지·CLI 임계값 3종·proto/migration/engine 0·ADR-0041 범위]·LAN 분산 워커 L7[제어 액션: /workers에서 풀 워커 drain/undrain·exclude·capacity override·label 운영자 제어·제어 3필드 PoolEntry in-memory(migration 0)·idempotent re-register 보존·drain/override가 L3~L5 가드 3경로+RunDialog 프리뷰에 동일규칙(effective_capacity_vus()/!drained) 반영=by-construction parity·exclude=additive proto Disconnect(field 5)로 워커 두 사이트 종료(재접속 0)+busy worker_disconnected fail-fast·무인증 trusted-LAN(R13)+경고 카피 보완통제·proto additive/migration/engine 0·ADR-0041 범위])까지 구현·머지 완료. 최신 = LAN 분산 워커 L7(ADR-0041 후속 — 읽기전용 L2 대시보드에 운영자 능동 제어 4종 추가: drain/undrain[새 배정 중단·진행 run 완주·되돌리기]·exclude[즉시 풀 제거+워커 깔끔 종료·busy run fail-fast]·capacity override[런타임 수동조정]·label[메모]. 제어 3필드(drained/capacity_override/label)는 PoolEntry in-memory(migration 0)·idempotent re-register 보존[R2]; drain·override가 L3~L5 가드 3경로+RunDialog 프리뷰에 effective_capacity_vus()/!drained 동일규칙 반영→화면숫자≡실제배정[R4 by-construction parity]. exclude만 워커를 실제 떼어내야 해 additive proto Disconnect[field 5]→워커 두 사이트(idle-wait 루프·forward_inbound) 프로세스토큰 cancel→재접속 없이 종료; busy면 기존 worker_disconnected fail-fast[run failed·terminal 정합]. REST PATCH/exclude[capacity 1..=1_000_000·label≤200·reason≤500 sanity·deserialize_some 3-level Option 부분갱신·404/400]·무인증 trusted-LAN 유지[R13]+모든 제어동작 위험경고 카피[exclude 차단형 alertdialog+busy run실패 경고]=로그인부재 보완통제. R14 락 규율[pool_exclude 캡처 락안·send/fail-fast 락밖·try_send]. proto additive only·migration/engine 0·byte-identical-off. STOP-gate 재개 후 fresh /clear 구현·subagent-driven 4 task d89e08f~4187a49[T1~T4 code-quality Opus 승격·5 fold-in]. handicap-reviewer APPROVE-WITH-NITS[4 불변식 source 대조: wire 1:1·R4 parity·R14 byte-identical+lock·R7 no-reconnect]·security APPROVE[6표면 clean: 시크릿 비노출·exclude 비-peer-triggerable·무인증=기존자세 동일·SSRF/injection 0]. 게이트 green: nextest 709/709·clippy 0·UI 1044. 라이브[실 pool 2워커 cap5,25]: PATCH 200/범위밖400/없음404/reason캡400·drain+override→409 achievable=4 & RunDialog 프리뷰 동시반영·exclude idle→프로세스exit·재등록0·exclude busy→run failed 단일terminal·Playwright 액션메뉴/배지/수동용량/drain확인/busy경고·Zod 0. LAN 분산 워커 제어 완결, 2026-06-22). 완료 슬라이스/기능 상세·함정 출처 → `docs/build-log.md`, A9 후보·연기 → `docs/roadmap.md`, ADR 인덱스 → 아래 [알아둘 결정들](#알아둘-결정들).** 다음 후보·연기 항목 → **`docs/roadmap.md`** (post-MVP1 단일 진입점, "다음 뭐 하지?"는 거기부터). 슬라이스/기능별 구현 결과·함정 출처(과거 이력) → **`docs/build-log.md`**. 결정 요약 → 아래 [알아둘 결정들](#알아둘-결정들), 전문 → `docs/adr/`. 디자인 → `docs/superpowers/specs/`, 구현 계획 → `docs/superpowers/plans/`. MVP 1단계 spec=`2026-05-27-handicap-mvp1-design.md`(슬라이스 1–6), 후속은 그 §4.5 메뉴에서 각자 새 spec/plan으로.
...
- **subagent 리포트 파일 경로를 `.git/worktrees/<wt>/sdd/`로 명시 + "리포트 `.md`를 worktree 루트에 쓰지 말 것·`git add` 금지" 못박기** (ko-common 롤업 T4): 안 하면 implementer가 `task-N-report.md`를 worktree 루트(tracked)에 써서 커밋돼 `ui/`-only diff 불변식을 깬다(루트 `task-4-report.md`가 `git add ui/src`에도 안 걸렸는데 커밋됨 → sdd로 보존 후 `git rm`+`commit --amend`로 복구). 후속 dispatch에 경로/금지 2줄 추가하니 재발 0.
- **완성도(전수 grep) 게이트는 orchestrator가 *직접 재실행*** (ko-common 롤업 T6): implementer가 "R1 grep clean/잔존 0"이라 보고해도 독립 재실행에서 잔존 6개 적발(verdict 배지 raw `PASS`/`FAIL`·삼항 속성 `title={c?"pass":"fail"}`·`max ms` 헤더 등). "subagent report 불신, 직접 실행한 테스트/빌드만 신뢰"의 grep-completeness 확장 — self-report한 grep 결과는 신뢰 천장이 낮다(grep 패턴/제외를 implementer가 자기에게 유리하게 적용). 완성도가 슬라이스 acceptance면 orchestrator가 같은 grep을 손수 돌려 잔존을 눈으로 판정.
