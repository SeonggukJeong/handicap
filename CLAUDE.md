...
**상태: MVP 1단계(슬라이스 1–9) 완료 + post-MVP1 영역 A(프리셋·멀티워커 fan-out·Parallel·리포트 export/insights)·B(환경·SLO criteria·리포트 깊이)·C(에디터 test-run)·D(부하모델·페이싱: 타임아웃/think-time/open-loop/stages/VU 곡선) + Run 스케줄러 + 영역 U(UX, ADR-0035) + 후속 다수(LAN 분산 워커 L1–L7·풀 운영 견고성/제어상태 영속화·Tauri 데스크톱[in-process, ADR-0042]·단일 self-contained exe[ADR-0039]·HAR→시나리오 가져오기·다중 데이터셋 바인딩·운영 상한 관리자·스텝 템플릿 관리·ko.common 한국어화·게이트-에러 한국어 매핑·디자인 시스템[ADR-0043]·RunDialog 간단/상세+목업 시각 충실도 재구성·비교 뷰 깊이/XLSX Δ 조건부 서식·트랜잭션 시간 분해·Run 라이브니스 G1/G3+stall 배지·closed-loop 곡선 fan-out 샤딩·결과화면/스텝 막대 폴리시·에디터 캔버스→아웃라인 재설계·에디터 레이아웃 후속 버그 수정·에디터 드래그 메커니즘 수리·에디터 YAML 모달 가져오기/내보내기·JSON 바디 캐스트 확장·에디터 테스트 흐름 칩 스트립·에디터 경계 드래그/re-parent·시나리오 삭제/이름 라이브 편집·에디터 공간·이름 QoL·Button-accent 색 이주·에디터 변수 도구 A·에디터 뷰포트 높이 floor·에디터 뷰포트 폴리시 v2·저장 안 됨 이탈 가드 등 — **전수 목록·구현 결과·함정 출처는 `docs/build-log.md`가 단일 소스**)까지 구현·머지 완료. 최신 = har-query-decode(HAR 가져오기 URL 안전 디코딩 — 신규 순수 모듈 `ui/src/import/urlDecode.ts`[escape-run 단위 치환·allow=공백+비ASCII 가시 문자·비허용은 원문 escape 슬라이스 보존·`TextDecoder(fatal, ignoreBOM:true)` — ignoreBOM이 선두 BOM desync·멱등 위반을 막는 load-bearing]로 스텝 URL·이름(`wireStep`, `parameterizeUrl` 뒤 적용)+미리보기 표시 2곳만 디코딩, 헤더/바디/`filters.ts`/dedup 데이터 경로 무변경·엔진 reqwest 재인코딩으로 와이어 byte-identical, UI-only·신규 ADR 없음·crates/proto/migration/`ui/src/api`/wire 0-diff·머지 `b4a77b4`, 2026-07-12): SDD 5 task(T1–T3 Sonnet impl·리뷰, T1 리뷰어 self-flag escalate→Opus 재패스가 BOM 버그 실측 적발→fix `039c18b`, T4 스윕·T5 라이브는 orchestrator 직접) 전부 Approved(Critical/Important 0)·최종 handicap-reviewer(Opus) APPROVED-WITH-NITS(비차단 2→roadmap §B19)·security-reviewer(Opus) APPROVED(업로드파싱 path-gate — 템플릿 토큰 %7B/%7D/%24 by-construction 도달 불가·제어문자 미디코딩·선형)·라이브 spec §6-3 Playwright+curl 전부 PASS(미리보기 디코딩 표시 실측·디코딩 run/인코딩-대조 run 와이어 REQ 69/69 byte-identical). 완료 슬라이스/기능 상세·함정 출처 → `docs/build-log.md`, 다음 작업(테마별 frontier+추천) → `docs/roadmap-status.md`(현황판·shortlist 대체), 후보 메뉴·연기 항목 상세 → `docs/roadmap.md`, ADR 인덱스 → 아래 [알아둘 결정들](#알아둘-결정들), 결정 전문 → `docs/adr/`. 디자인 → `docs/superpowers/specs/`, 구현 계획 → `docs/superpowers/plans/`. MVP 1단계 spec=`2026-05-27-handicap-mvp1-design.md`(슬라이스 1–6), 후속은 그 §4.5 메뉴에서 각자 새 spec/plan으로.**
