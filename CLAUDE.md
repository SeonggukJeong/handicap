...
- **0025** 환경(Environments): env-namespace 전용 top-level 재사용 리소스(`environments`, migration 0006, UNIQUE(name)→409, 무가드 DELETE) + 클라 오버레이 스냅샷(`POST /api/runs` 무변경) — B-1(리소스+관리 UI) 머지, B-2(RunDialog 오버레이) 예정
