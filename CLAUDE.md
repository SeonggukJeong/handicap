...
**상태: MVP 1단계(슬라이스 1–9) + post-MVP1 영역 A(프리셋·멀티워커 fan-out·Parallel 노드·리포트 export/insights)·B(환경·SLO criteria·리포트 깊이)·C(에디터 test-run)·D(부하모델·페이싱: 타임아웃/think-time/open-loop/stages) + Run 스케줄러 + B7-C 레이턴시 단계 분해 + Parallel per-branch breakdown + 영역 U(UX 초보자 친화 재설계, ADR-0035) + 스텝 템플릿(저장/복사-삽입 스냅샷, ADR-0036, 2026-06-12 완결: `step_templates` 리소스+migration 0015+에디터 저장/삽입 모달, 삽입 시 ULID 재발급) + skip/todo UI 테스트 정리(2026-06-12: `it.todo` 18→0, 특성화 9 구현·근거 삭제 9, 스위트 762 pass — codex 후속 §B5 전부 완결) + closed-loop VU 곡선(vu_stages, ADR-0037, 2026-06-13 완결: park-gate 격리 엔진 함수 `run_scenario_vu_curve` + graceful/immediate ramp_down 노브 + proto 12·13 + 검증 ①–⑨ + UI closed+curve 활성화, 단일워커 v1, 마이그레이션 0) + active-VU 초당 시계열(ADR-0037 follow-up, 2026-06-14 완결: migration 0016 `run_active_vu_metrics` + `MetricFlush.active_vu_samples`(7번째 벡터) + `ReportJson.active_vu_series` → UI `ActiveVuChart`(목표 점선/실제 실선), 곡선만 emit·비-곡선 byte-identical·처리량 무회귀) + A9 부하 생성기 포화 인사이트(`load_gen_saturated`, ADR-0028 범위 내·신규 ADR 불필요, 2026-06-14 완결: open-loop `dropped>0`이면 `derive_insights`가 관측 천장=peak per-second throughput·count=dropped로 인사이트 emit, `order_rank` rank 3, UI InsightPanel 초보자 문구, `dropped==0`이면 byte-identical, proto/migration/engine/worker 무변경) + A9 사이징 권장(Little's Law, ADR-0028 범위 내·신규 ADR 불필요, 2026-06-14 완결: `dropped>0`이면 `required=ceil(target_rps×p50_sec)`를 설정 `max_in_flight`와 비교해 `cause`∈{slots(권장 max_in_flight 숫자 제시)/capacity(올려도 무익=워커 CPU/SUT 한계)}/폴백(p50==0·profile 부재) 분기, `Insight`에 optional `recommended`/`cause` 2필드 가산 + UI `InsightPanel.actionFor` cause 분기 렌더, 유효목표=`target_rps` or stages-peak(report.rs 주입), proto/migration/engine/worker/골든 fixture 무변경·`dropped==0`·폴백 byte-identical) + step-level + 일반 연산자 SLO criteria(ADR-0028 범위 내·신규 ADR 불필요, 2026-06-14 완결: 가산 `step_criteria: Vec<Criterion{metric,op,threshold,target}>` 8 metric×max/min·http-leaf target 필수, `evaluate_criteria`에 `steps` 파라미터로 step 행 평가[미존재/`count==0` skip·전부-skip→verdict None·rate 분모 공유헬퍼] + `validate_step_criteria_targets` http-leaf walk 5 호출부 배선[run/preset create·update/schedule gate/fire] + UI `StepCriteriaFields`·VerdictPanel step명·VerdictPanel/Badge React key 충돌 수정, 엔진/워커/proto/migration/export 무변경·`step_criteria` 빈·`target` None serde 생략→byte-identical) + 닫힌 루프 create-time VU 사이징 헬퍼(roadmap §A9 연기 2건=create-time 사이징 힌트+closed-loop `vus` 권장, ADR 불필요·순수 UI 슬라이스, 2026-06-14 완결: RunDialog 닫힌+고정에 목표 RPS→권장 VU 역산[Little's Law 선형스케일 `targetVus=priorVus×targetRps/priorRps`], 처리량 3계층=최근 closed run 앵커[`summary.rps/vus`]→test-run 측정[`R/total_ms`·truncated 거부]→수동 추정, 권장값=하한, `sizing.ts`(순수)+`VuSizingHelper`(co-located `usePriorClosedRunAnchor`·async 1회 시드 `touchedRef` race 가드)+`LoadModelFields` optional prop 4종[`onApplyVus` 있을 때만 렌더→RunDialog만 전달=공유 ScheduleForm 부재], 엔진/워커/proto/migration/controller 무변경·run 페이로드 byte-identical=머지 diff ui/ 한정) + 열린 루프 슬롯(max_in_flight) create-time 사이징 헬퍼(roadmap §A9 연기 1건=open-loop create-time 슬롯 힌트, ADR 불필요·순수 UI 슬라이스, 2026-06-15 완결: 위 닫힌 루프 헬퍼의 open-loop 대칭이자 A9 사후 `load_gen_saturated`의 거울상, RunDialog open+고정에 목표 RPS→권장 `max_in_flight=ceil(target_rps×p50_sec)`[사후 `insights.rs:224` `required`와 수식 1:1], 지연 3계층=최근 종료 open-loop run 앵커[`summary.p50_ms`·`p50>0` 가드]→수동 추정→무부하 test-run[`total_ms/R`·truncated 거부], 권장값=하한, `sizing.ts`(`recommendSlots`·`pickLatestOpenRun`=`is_open_loop` 양성식)+자족 `SlotSizingHelper`(co-located `usePriorOpenRunAnchor`·CAP 10000 비차단 경고)+`LoadModelFields` optional `onApplyMaxInFlight`[open+fixed·`onApplyMaxInFlight && sizingScenarioId!==undefined`일 때만 렌더→RunDialog만 전달=ScheduleForm 부재], 엔진/워커/proto/migration/controller 무변경·run 페이로드 byte-identical=머지 diff ui/ 한정·라이브 수식 parity 11=11) + open+curve 슬롯 힌트(roadmap §A9·위 open+fixed 슬롯 헬퍼의 곡선 확장, ADR 불필요·순수 UI 슬라이스, 2026-06-15 완결: open+curve에서 stages **피크 단계 목표** `peakStageTarget=max(stage.target)`를 유효 목표로 권장 `max_in_flight=ceil(peak×p50_sec)`[`report.rs:616-621` 곡선 유효목표 `target_rps.or_else(||stages.max())` ↔ `insights.rs:224` `required`와 수식 1:1], 신규 컴포넌트 0=기존 `SlotSizingHelper`에 `peakBased?:boolean` 1 prop으로 문구 2개만 곡선 변형[`formulaPeak`"최고 단계 목표 N RPS"/`needTargetCurve`], `LoadModelFields` open+curve arm에 `peakStr=useMemo(peakStageTarget(stages))`+open+fixed 동형 게이트 렌더[closed+curve는 `loadModel==="closed"` 브랜치라 구조적 미렌더·신규 prop 0], 엔진/워커/proto/migration/controller·Zod 와이어 무변경·run 페이로드 byte-identical=머지 diff ui/ 7파일 한정·라이브 parity 11=11=ceil(200×55/1000))까지 전부 구현·머지 완료.** 다음 후보·연기 항목 → **`docs/roadmap.md`** (post-MVP1 단일 진입점, "다음 뭐 하지?"는 거기부터). 슬라이스/기능별 구현 결과·함정 출처(과거 이력) → **`docs/build-log.md`**. 결정 요약 → 아래 [알아둘 결정들](#알아둘-결정들), 전문 → `docs/adr/`. 디자인 → `docs/superpowers/specs/`, 구현 계획 → `docs/superpowers/plans/`. MVP 1단계 spec=`2026-05-27-handicap-mvp1-design.md`(슬라이스 1–6), 후속은 그 §4.5 메뉴에서 각자 새 spec/plan으로.
