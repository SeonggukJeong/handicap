...
**cold-build flake: engine/worker를 바꾼 커밋은 pre-commit의 `cargo test --workspace`가 controller e2e(`loop_e2e_inner_step_counts`·`full_slice_1_e2e`)에서 워커 바이너리 race로 flake날 수 있다** (S-A) — 증상: `target/debug/worker` ENOENT(`No such file or directory` → dispatch failed → run failed → 테스트 `unwrap()` None panic) 또는 SIGKILL(sig 9). **진짜 회귀 아님**(codex-followups의 dispatch-fail→run-failed 동작과 겹쳐 오인하기 쉬움). 대응: 커밋 전 `cargo build -p handicap-worker && cargo build --workspace`로 warm한 뒤 커밋하고, flake나면 동일 커밋을 warm 상태로 재시도.
