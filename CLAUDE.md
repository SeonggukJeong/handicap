...
- **0031** Open-loop / arrival-rate 실행 모델: opt-in(`target_rps`)→격리된 `run_scenario_open_loop`(closed-loop byte-identical) + 균등 틱 스케줄러 + `max_in_flight` 재사용 슬롯풀(슬롯=vu_id, jar 슬롯-지속) + drop+`dropped` 카운터(migration 0009 `runs.dropped`, advisory-only) + 단일워커 v1(fan-out 연기) + 노브충돌(ramp/run-level think_time) 400
- **0032** 다단계 ramp (open-loop 레이트 곡선): stages=target_rps+duration generic 대체(상호배타, max_in_flight 직교 필수), 시작 rate=0(k6) + fixed|curve UI, 순간-rate 스케줄러 v1(piecewise-linear), is_open_loop predicate로 판별 통일, 마이그레이션 0(profile_json)·메트릭/proto 무변경·stages 없으면 byte-identical. closed-loop stages·모드선택기·정확적분·fan-out 연기.
- **0033** Parallel 노드 (동시 분기 fan-out): `type: parallel` 한 VU가 분기를 `join_all`로 동시 실행(wait-all, 공유 jar/client) + `{{branch.var}}` key-origin 네임스페이스 merge-back(선언 순서) + top-level-only·http-only 분기(two-tier Zod) + trace 순차·동일 merge + insights 1 arm + 캔버스 세로 레인 + Inspector 분기 CRUD. **proto·워커·migration·메트릭(per-step 재사용) 무변경, 핫 flat-http byte-identical.** 그룹/페이지 레이턴시·per-branch breakdown·중첩·open-loop 상호작용·분기 fail-fast 연기.
