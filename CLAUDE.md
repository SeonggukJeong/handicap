...
- **모든 리뷰-수정 루프는 *유한*하다 — "APPROVE까지"는 무한 루프 footgun** (보안 게이트 추가 세션): subagent-driven 스킬의 "repeat until approved"를 곧이곧대로 두면 reviewer가 nit을 끝없이 생성할 때 안 끝난다. **종료 규칙**: ① **blocking급 finding(Critical/High·spec invariant 위반·와이어 깨짐)만 차단** — Minor/Info/하드닝은 기록하고 진행(다음 라운드 트리거 안 함). ② `receiving-code-review`로 타당성 먼저 판정 — 틀림/범위밖은 근거 1줄로 **기각**(맹종이 루프를 늘린다). ③ **round cap 2–3**: 그 안에 blocking 미수렴(같은 류 반복·새 blocking 무한 생성)이면 **루프 중단 + 사용자에게 disagreement 요약 escalate**(spec-plan-reviewer 루프의 start-slice §4 상한과 동형). `finish-slice §0` 보안 게이트가 이 규칙을 명문화한 사례.
