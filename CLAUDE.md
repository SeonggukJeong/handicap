...
**상태: MVP 1단계(슬라이스 1–9) 완료 + post-MVP1 영역 A(프리셋·멀티워커 fan-out·Parallel·리포트 export/insights)·B(환경·SLO criteria·리포트 깊이)·C(에디터 test-run)·D(부하모델·페이싱: 타임아웃/think-time/open-loop/stages/VU 곡선) + Run 스케줄러 + 영역 U(UX, ADR-0035) + 후속 다수(LAN 분산 워커 L1–L7·풀 운영 견고성/제어상태 영속화·Tauri 데스크톱[in-process, ADR-0042]·단일 self-contained exe[ADR-0039]·HAR→시나리오 가져오기·다중 데이터셋 바인딩·운영 상한 관리자·스텝 템플릿 관리·ko.common 한국어화·게이트-에러 한국어 매핑·디자인 시스템[ADR-0043]·RunDialog 간단/상세+목업 시각 충실도 재구성·비교 뷰 깊이/XLSX Δ 조건부 서식·트랜잭션 시간 분해·Run 라이브니스 G1/G3+stall 배지·closed-loop 곡선 fan-out 샤딩·결과화면/스텝 막대 폴리시·에디터 캔버스→아웃라인 재설계·에디터 레이아웃 후속 버그 수정·에디터 드래그 메커니즘 수리·에디터 YAML 모달 가져오기/내보내기·JSON 바디 캐스트 확장·에디터 테스트 흐름 칩 스트립·에디터 경계 드래그/re-parent·시나리오 삭제/이름 라이브 편집·에디터 공간·이름 QoL·Button-accent 색 이주·에디터 변수 도구 A·에디터 뷰포트 높이 floor·에디터 뷰포트 폴리시 v2·저장 안 됨 이탈 가드·HAR 쿼리 안전 디코딩·open-loop 슬롯 사이징 교정·open-loop 단위 표면화·시나리오 기본 think time·open-loop think 검증/무시 토글(§B21)·graceful ramp-down 상한(§B9)·에디터 test-run 데이터셋 바인딩 등 — **전수 목록·구현 결과·함정 출처는 `docs/build-log.md`가 단일 소스**)까지 구현·머지 완료. 최신 = pair-input-blur-commit(§B13 correctness-bug — 짝(min/max) 입력에서 min→max 포커스 이동의 암묵 blur가 중간 쌍({new_min, old_max})을 커밋해 `min>max` revert로 떨궈 방금 친 값이 사라지던 결함 수정 — `200/500`에서 `1000`·`2000` → 저장은 `200–2000`. 공유 훅 `useThinkTimePair`가 4곳에 복제된 draft 상태기계를 흡수하고 그 안에서 커밋 경계를 고친다[`relatedTarget`이 짝의 반대 칸이면 보류] + ESC는 blur 없이 언마운트하므로 현황판 닫기 직전 동기 `activeElement.blur()` flush(R5). 유효성 규칙 `resolveThinkDraft`는 **0-diff** — 바뀐 건 커밋이 발화하는 *시점*뿐. UI-only·와이어 무변경·ko 문구 0. **수렴**: `resolveThinkDraft` 소비처가 4사본→1곳. **리뷰가 걷어낸 2건**: 브리프가 지정한 `RefObject<HTMLInputElement | null>`이 `tsc -b`를 깨뜨림(리뷰어가 최소 재현으로 독립 검증)·내가 추가 요청한 `resetKey` 테스트가 **값-변경 경로와 혼동**되어 dep을 빼도 통과(같은 값·다른 id + 미커밋 draft로 재작성 후 이빨 실증). 게이트 2026 green·보안 N/A(grep+판단)·**라이브 10/10 PASS**(4 표면·ESC 후 재오픈·✕·백드롭·US2 무음 revert·Inspector 상세 모달·양쪽 진입 화면). 잔여: `bulkValid` 인라인 경계상수·alt-tab 경로(spec §7.2 수용된 한계)·`scenarioHasThink`의 `{0,0}` 비대칭(부하 모델 슬라이스로 연기). 머지 300e294, 2026-07-20): 완료 슬라이스/기능 상세·함정 출처 → `docs/build-log.md`, 다음 작업(테마별 frontier+추천) → `docs/roadmap-status.md`(현황판·shortlist 대체), 후보 메뉴·연기 항목 상세 → `docs/roadmap.md`, ADR 인덱스 → 아래 [알아둘 결정들](#알아둘-결정들), 결정 전문 → `docs/adr/`. 디자인 → `docs/superpowers/specs/`, 구현 계획 → `docs/superpowers/plans/`. MVP 1단계 spec=`2026-05-27-handicap-mvp1-design.md`(슬라이스 1–6), 후속은 그 §4.5 메뉴에서 각자 새 spec/plan으로.**
