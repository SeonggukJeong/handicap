...
**상태: MVP 1단계(슬라이스 1–9) 완료 + post-MVP1 영역 A(프리셋·멀티워커 fan-out·Parallel·리포트 export/insights)·B(환경·SLO criteria·리포트 깊이)·C(에디터 test-run)·D(부하모델·페이싱: 타임아웃/think-time/open-loop/stages/VU 곡선) + Run 스케줄러 + 영역 U(UX, ADR-0035) + 후속 다수(LAN 분산 워커 L1–L7·풀 운영 견고성/제어상태 영속화·Tauri 데스크톱[in-process, ADR-0042]·단일 self-contained exe[ADR-0039]·HAR→시나리오 가져오기·다중 데이터셋 바인딩·운영 상한 관리자·스텝 템플릿 관리·ko.common 한국어화·게이트-에러 한국어 매핑·디자인 시스템[ADR-0043]·RunDialog 간단/상세+목업 시각 충실도 재구성·비교 뷰 깊이/XLSX Δ 조건부 서식·트랜잭션 시간 분해·Run 라이브니스 G1/G3+stall 배지·closed-loop 곡선 fan-out 샤딩·결과화면/스텝 막대 폴리시·에디터 캔버스→아웃라인 재설계·에디터 레이아웃 후속 버그 수정·에디터 드래그 메커니즘 수리·에디터 YAML 모달 가져오기/내보내기·JSON 바디 캐스트 확장·에디터 테스트 흐름 칩 스트립·에디터 경계 드래그/re-parent·시나리오 삭제/이름 라이브 편집·에디터 공간·이름 QoL·Button-accent 색 이주·에디터 변수 도구 A·에디터 뷰포트 높이 floor·에디터 뷰포트 폴리시 v2·저장 안 됨 이탈 가드·HAR 쿼리 안전 디코딩·open-loop 슬롯 사이징 교정·open-loop 단위 표면화·시나리오 기본 think time·open-loop think 검증/무시 토글(§B21)·graceful ramp-down 상한(§B9)·에디터 test-run 데이터셋 바인딩·생성 변수(dynamic-vars)·시나리오 공유 메모(scenario-notes)·실행 전 신뢰도 preflight(scenario-preflight)·HAR host-환경 힌트(har-host-env-hint)·바인딩 변수명 가시성(binding-varname-visibility)·릴리즈 위생(release-hygiene) 등 — **전수 목록·구현 결과·함정 출처는 `docs/build-log.md`가 단일 소스**)까지 구현·머지 완료. 최신 = release-hygiene(릴리즈 위생 — 버전 신원 + 릴리즈 파이프라인·노트, roadmap §B24+§B25 묶음[사용자 명시 선택]. 루트 `[workspace.package] version="0.7.0"` 단일 소스 + crates 5개 상속 → clap `--version`(controller·worker)·시작 로그 5곳·`GET /api/version`(단일 `version` 필드, Zod `.strict()`와 1:1)·UI 헤더 배지로 표면화. 릴리즈 CI를 `preflight → build×2 → publish` 4잡으로 갈라 게시 실패 시 **publish만 재실행**(18분→수십 초, 구 30분 폴링 소멸)하고, 판정·게시 로직을 레포 스크립트 2개(`check-release-versions.sh` 14항목·`publish-release.sh` 노트 4분기+유한 재시도)에 둬 **로컬 양방향 실행**으로 증명 — CI-only 기능의 유일한 검증 수단이고 실패 단위가 18분 이터레이션이라 이 설계가 핵심. `just bump-version`(사람이 맞추는 3파일 + 생성물 락 2개 + 정합 검사, semver assert로 `v` 접두 오타 차단) + v0.6.0 소급 노트 + 절차 문서. 리뷰가 잡은 실질 결함 2건은 둘 다 **거짓을 말하는 코드**: 게시 스크립트의 빈-본문 판정이 `$( )`의 종료 상태를 버려 API 순간 장애를 "빈 본문"으로 오판해 **손으로 쓴 노트를 자동 초안으로 덮어썼고**[스크립트에서 유일하게 `retry` 밖인 gh 호출], 같은 분기 로그는 읽기 실패에도 "기존 본문 있음"이라 거짓 진단. **orchestrator 오류 2건**: `--is-ancestor c v0.6.0`은 **구간 소속을 증명하지 않아**[더 오래된 커밋이 전부 통과] v0.5.0에 이미 공지된 항목이 v0.6.0 노트에 들어갔고(리뷰 적발·절 삭제), bundle 게이트를 `| tail`로 돌려 종료코드를 마스킹[레포가 명시 문서화한 함정 — 파이프 없이 재실행: 35 바이너리/591 passed/0 failed]. 라이브 **US1·US1'·US2 PROVEN**[헤더 gap 8px·`headerChildren=2`로 중앙-부양 실패모드 차단·US1'은 **양성 대조를 먼저** 잡아 공허 통과 방지·Zod raw 에러 0], **US3도 PROVEN**(사용자 승인 후 `gh release edit v0.6.0 --notes-file …` 실행 → 공개 본문 0자→2590자·에셋/태그 무변경·v0.7.0 무영향; 최종 리뷰가 그 게시 경로 부재를 적발한 덕에 닫혔다), US4·US5는 스텁·양방향 실행으로 대체 증명 · 머지 6c80592, 2026-07-30): 완료 슬라이스/기능 상세·함정 출처 → `docs/build-log.md`, 다음 작업(테마별 frontier+추천) → `docs/roadmap-status.md`(현황판·shortlist 대체), 후보 메뉴·연기 항목 상세 → `docs/roadmap.md`, ADR 인덱스 → 아래 [알아둘 결정들](#알아둘-결정들), 결정 전문 → `docs/adr/`. 디자인 → `docs/superpowers/specs/`, 구현 계획 → `docs/superpowers/plans/`. MVP 1단계 spec=`2026-05-27-handicap-mvp1-design.md`(슬라이스 1–6), 후속은 그 §4.5 메뉴에서 각자 새 spec/plan으로.**
