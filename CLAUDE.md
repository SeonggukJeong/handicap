...
**상태: MVP 1단계(슬라이스 1–9) + post-MVP1 영역 A(프리셋·멀티워커 fan-out·Parallel·리포트 export/insights)·B(환경·SLO criteria·리포트 깊이)·C(에디터 test-run)·D(부하모델·페이싱: 타임아웃/think-time/open-loop[+멀티워커 fan-out]/stages/VU 곡선) + Run 스케줄러 + 영역 U(UX, ADR-0035) + 후속 다수(스텝 템플릿·active-VU 시계열·A9 용량/사이징 인사이트·step-level SLO·create-time 사이징 헬퍼 4종[닫힌 VU·열린 슬롯 fixed/curve·worker_count]·mean 지연 프록시[사이징 p50→mean·parity]·A9 포화 원인 귀속[loadgen/sut+지속최대RPS·onset]·CSV/compare 인사이트 표면화·HAR→시나리오 가져오기·다중 데이터셋 바인딩[한 run에 N개 독립]·운영 상한 관리자 화면[op-config 상한 6종 런타임 가변]·단일 self-contained 바이너리[Windows 경량 데스크톱 배포·bundle feature off=byte-identical]·응답기반 extract 작성[test-run 응답 클릭→스텝 extract·UI-only]·스텝 템플릿 관리 페이지+삽입 시 변수 파라미터화[/templates 관리(EnvironmentsPage 미러·생성 없음)+토큰 keep/rename/literal 2-phase 단일 Modal·UI-only]·ko.common 한국어화 일괄 롤업[ui/src 잔존 영어/인라인 한국어 전수→ko.ts 카탈로그·UI-only]·에디터 test-run UX 정리+스킴-없는 URL 검출[헤더 중복 버튼 제거→test-run 단일 섹션 통합·검증배너 스킴-없는 host URL 일반화·UI-only]·게이트-에러 한국어 매핑[Zod discriminator/enum/unrecognized-key/min 클래스→ko.editor.gate* 매핑·실-Zod 3.25.76 드리프트 가드·model.ts 0-diff·UI-only]·Tauri 데스크톱 셸[bundle controller 사이드카 래핑→네이티브 창+Windows 인스톨러·desktop/ 신규(자체 [workspace]로 cargo 워크스페이스 밖)·crates/ui 0-diff])까지 구현·머지 완료. 최신 = Tauri 데스크톱 셸(ADR-0040 옵션 B 접근1 — 기존 bundle controller exe를 사이드카로 직접 spawn[tokio Command, plugin-shell 아님]→controller 로그에서 실제 REST 포트 파싱[**ANSI strip이 load-bearing**: tracing_subscriber가 파이프엔 NO_COLOR 무시·ANSI 출력 → `parse_rest_port`가 CSI 제거 후 매칭, 실-ANSI fixture 드리프트 가드]→`/api/health`==`ok` 확인 후에만 네이티브 창을 `http://127.0.0.1:<port>`로 navigate, 창 닫힘 시 `RunEvent::Exit`→`process_group`+killpg[Unix SIGTERM→SIGKILL]/Job Object[Windows KILL_ON_JOB_CLOSE]로 controller+손자 워커 트리째 종료[고아 0]. 신규 `desktop/` Tauri v2 크레이트는 자체 빈 `[workspace]`로 루트 워크스페이스 밖[R5·cargo 게이트 무영향]·`crates/**`+`ui/src` 0-diff[R4]·`SpawnConfig` localhost 기본[R6]·`ControllerBackend{base_url,shutdown}` 추상[R7 접근2/LAN 전방호환]. subagent-driven 6 task[스캐폴드+`launch.rs` 순수글루→`backend.rs` 생명주기→셸배선+splash→런북+`tauri build`→macOS 라이브→docs] 독립 green 커밋 `2109b1e`~`e1e16af`, T2 code-quality Opus 승격[spawn+동시성+unsafe libc, Important 포트-대기 타임아웃 트리 누수 fix]. handicap-reviewer APPROVE[R1–R14·R4 와이어 0-diff·R5 워크스페이스 밖]+security-reviewer APPROVE-WITH-FIXES[`csp:null`=controller UI 자체 `<meta>` CSP·웹과 byte-identical→문서화 / env-override 런북 노트]. macOS 라이브 R2/R3/R8 ✅[실 release bundle controller+손자 워커 killpg 고아 0·run 완료 rps 91·실 ANSI 파이프 포트 파싱], R1 창렌더/R9 WebView는 GUI 세션 연기[Windows 갭 동일], 2026-06-19). 완료 슬라이스/기능 상세·함정 출처 → `docs/build-log.md`, A9 후보·연기 → `docs/roadmap.md`, ADR 인덱스 → 아래 [알아둘 결정들](#알아둘-결정들).** 다음 후보·연기 항목 → **`docs/roadmap.md`** (post-MVP1 단일 진입점, "다음 뭐 하지?"는 거기부터). 슬라이스/기능별 구현 결과·함정 출처(과거 이력) → **`docs/build-log.md`**. 결정 요약 → 아래 [알아둘 결정들](#알아둘-결정들), 전문 → `docs/adr/`. 디자인 → `docs/superpowers/specs/`, 구현 계획 → `docs/superpowers/plans/`. MVP 1단계 spec=`2026-05-27-handicap-mvp1-design.md`(슬라이스 1–6), 후속은 그 §4.5 메뉴에서 각자 새 spec/plan으로.
