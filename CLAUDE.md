...
- **0044** 에디터 1차 표현 캔버스→아웃라인: React Flow 팬 캔버스를 세로 인터랙티브 아웃라인(`FlowOutline` HTML 트리)으로 교체 + 디테일 편집기 1fr + 변수 접기 + YAML 양방향 모달 + dnd-kit 그룹내 드래그 재정렬(`resolveDragEnd`→`moveStep`, 경계 넘기=슬라이스3 연기) + `@xyflow/react` 제거. 양방향 sync(0003/0015) 모델 유지(아웃라인=같은 store 위 새 뷰)·모델/wire byte-identical(에디터 구조 재설계 1/3)
