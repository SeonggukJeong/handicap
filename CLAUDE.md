...
**상태: Slice 8a(본문 템플릿팅) + 8b(데이터셋 리소스) 구현 완료·master 머지.** 다음 = Slice 8c(데이터셋 바인딩+주입). (MVP 1단계 = 슬라이스 1–6, Slice 7부터는 노드 종류 확장의 후속. Slice 8 = data-driven, 8a/8b/8c 분할.) 디자인 문서 → `docs/superpowers/specs/`. 구현 계획 → `docs/superpowers/plans/`. 결정 기록 → `docs/adr/`. **MVP 1단계 spec(`2026-05-27-handicap-mvp1-design.md`)은 슬라이스 1–6으로 전부 구현됨 — 후속은 그 spec §4.5 메뉴(노드 종류 확장 → 멀티 워커/HPA → LoadRunner급 리포트)에서 각자 새 spec/plan으로 나온다.**
