...
- **A3a 메트릭은 keep-first(레이턴시 부정확) — A3b 까지** (A3a): `ingest_metrics`는 step_id/loop_index/branch 로 누적(counts 는 N 워커 합이 정확). 하지만 같은 `(run_id, step_id, ts_second)` HDR 행은 워커별로 들어와 마지막 것이 이전을 덮는다 → 퍼센타일 부정확. 워커별 머지(`run_metrics` PK 에 worker_id + 읽기 시점 HDR merge, migration 0008)는 A3b. **A3a 단독 출하 금지 — A3a+A3b 한 세트.**
- **`dispatcher.cleanup()`은 아직 아무도 안 부른다 (A3c 로 연기)** (A3a): 완료/abort 어느 경로에서도 cleanup 미호출(코디네이터가 dispatcher 핸들 안 듦). subprocess self-terminate, K8s Job 은 TTL+ownerRef GC. K8s Indexed Job 재작성(A3c)에서 cleanup 배선. `dispatch(run_id, worker_count)` — subprocess 는 N children(각 distinct ULID `--worker-id`), K8s 는 `worker_count>1` 이면 warn + 단일 Job(Indexed=A3c, `build_job_spec` 무변경).
