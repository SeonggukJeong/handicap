...
- **`just e2e-kind` 실행 현실: 클러스터 자동 생성 + 콜드 ~10–20분 + 성공 후 클러스터 잔존** (unique 라이브검증): `scripts/e2e-kind.sh`→`deploy-kind.sh`가 kind 클러스터 없으면 `kind create cluster`로 만들고 **Docker 안에서 release 빌드**(controller+worker; deps 캐시돼도 새 소스는 fresh 컴파일이라 콜드 전체 ~10–20분). Bash 툴 최대 timeout(10분)을 넘으므로 **detached 실행**(`nohup just e2e-kind >/tmp/e2e-kind.log 2>&1 &`) + Monitor로 terminal 폴(`grep "e2e-kind PASSED"` 또는 `! pgrep -f scripts/e2e-kind.sh`). 성공해도 **클러스터·이미지 캐시는 잔존**(재실행 빠름) — 정리는 `kind delete cluster --name handicap`. 단언: `mode=Indexed completions=2`(run 진행 중 관측) + driver 성공 + report `count`/`steps`.
