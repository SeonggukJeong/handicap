...
![시나리오 에디터 — 변수 패널(생성 변수·추출 변수), 드래그-드롭 아웃라인(반복 포함), 스텝 편집 폼, 실행 전 신뢰도 칩](docs/images/editor.png)
...
- **타임아웃** — run 전역 + 스텝별 오버라이드, 연결(connect) 단계 전용 타임아웃 별도 설정.
- **VU 사이징 도우미** — "목표 RPS를 내려면 VU가 몇 개 필요한가"를 직전 실행의 실측 RPS 기준으로 다이얼로그가 직접 권장합니다. 감이 아니라 측정으로 크기를 정합니다.

![실행 설정(상세 모드) — 부하 모델 선택 타일, 직전 실행 기준 크기 칩, 실측 RPS 기반 VU 사이징 도우미, 데이터셋 바인딩](docs/images/run-dialog.png)
...
- **생성 변수**: 날짜(±오프셋·서식)·랜덤 정수·UUID·랜덤 문자열을 반복마다 새로 만들어 주입 — 에디터가 예시 값을 미리 보여줍니다
...
- **HAR 가져오기**: 브라우저 개발자도구로 녹화한 트래픽(HAR)을 시나리오로 변환 — 녹화된 호스트를 환경 변수로 바꾸도록 안내까지
- **스텝 템플릿**: 자주 쓰는 스텝(예: 사내 SSO 로그인)을 템플릿으로 등록해 복사-삽입
- **공유 메모**: "이 시나리오는 스테이징 전용" 같은 주의점을 시나리오에 메모로 남겨 팀원에게 전달 — YAML에 함께 저장됩니다
- **시나리오 디버깅**: 에디터에서 **단발 테스트 실행** — 부하 없이 1회 실행해 실제 요청/응답 본문과 추출된 변수를 확인한 뒤 본 시험에 들어갑니다. 데이터셋 행을 물린 채로도 실행됩니다.

### 7. 시험 자체를 의심한다 — 신뢰도 점검 내장

부하 테스트에서 가장 위험한 것은 실패한 시험이 아니라 **믿으면 안 되는 결과를 믿는 것**입니다. Handicap은 이를 도구 차원에서 막습니다.

- **실행 전 preflight** — 공급되지 않는 변수, 응답 검증(assert) 부재 같은 "결과를 오염시키는 상태"를 에디터가 상시 신뢰도 칩으로 등급화하고, 문제가 있으면 실행 다이얼로그가 고칠 곳을 짚어줍니다.
- **해석 가능성 표시** — 응답 검증이나 SLO 기준 없이 돌린 run은 리포트에 그 사실이 함께 표시됩니다. "에러 0"이 정말 정상이라는 뜻인지, 검증이 없어서 0인지 도구가 구분해 줍니다.
- **부하 정직성** — 설정한 부하와 다르게 발사될 상황(워커 용량 부족 등)에서는 조용히 진행하지 않고, 실제 부하가 어떻게 달라지는지 설명한 뒤 확인을 받습니다.

### 8. 리포트가 곧 결과 보고서

시험이 *도는 동안*에는 라이브 궤적(초당 요청·에러 흐름)으로 진행 상황을 확인합니다:

![실행 중 화면 — 라이브 RPS·에러 궤적 차트, 진행 중 집계 카드](docs/images/live-run.png)
...
- **한국어 핵심 인사이트** — "5xx가 응답의 1.5%", "에러의 64%가 특정 스텝에 몰림" 같은 요지를 리포트가 문장으로 요약합니다. 전송 실패는 연결 거부·타임아웃·DNS 실패 등 **8종으로 분류**되고 발생 시점 분석으로 원인 후보까지 짚습니다. 조언은 실질적일 때만 — 정상이면 조용합니다.
...
서버를 튜닝한 뒤 같은 시험을 다시 돌리면, 비교 화면이 개선 폭을 색으로 보여줍니다:

![런 비교 — 두 run의 p50/p95/p99·RPS·에러율 차이를 색으로 강조, 시계열 오버레이](docs/images/compare.png)

> 본격 실시간 분석 대시보드는 의도적으로 뺐습니다 — 시험 *중* 깊은 관찰은 이미 쓰고 계신 APM이 더 잘합니다. Handicap은 실행 중엔 가벼운 라이브 궤적까지만 보여주고, 시험 *후* 판정과 보고에 집중합니다.

### 9. PC 한 대에서 시작해, 필요한 만큼 확장
...
3. **부하 발사** — 실행 버튼을 누르면 Run 다이얼로그가 열립니다. 간단 모드에서 크기 칩(직전 실행 대비 0.5×/1×/2×)과 시간만 고르면 되고, 상세 모드에서 ramp-up·VU 곡선·목표 RPS·think time·SLO 기준까지 조정할 수 있습니다. 실행이 시작되면 라이브 궤적 차트로 진행 상황이 보입니다.
4. **리포트 확인** — 종료되면 리포트가 생성됩니다. 시계열 그래프, 스텝별 p95/p99, 상태코드 분포, SLO 합격/불합격 배지, 핵심 인사이트를 확인하고, 필요하면 CSV/XLSX로 내보냅니다.
...
variables:
  ...
  trace_id: {gen: uuid}      # 생성 변수 — 반복마다 새 값 (날짜·랜덤 정수·문자열도 가능)
steps:
...
- **실시간 관찰은 라이브 궤적 차트까지** — 실행 중 화면은 RPS·에러 흐름 수준입니다. 본격 실시간 분석 대시보드는 설계상 제외 — APM을 병행하세요.
...
| [docs/adr/](docs/adr/) | 아키텍처 결정 기록 51건 (왜 이렇게 만들었는가) |
