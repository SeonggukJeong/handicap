...
function setHooks(opts: {
  ...
  (useRunReport as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
    data: opts.mean != null ? { summary: { mean_ms: opts.mean } } : undefined,
  });
...
describe("SlotSizingHelper", () => {
  it("앵커(mean): 권장 슬롯 + mean 출처 문구 + 계산식", () => {
    setHooks({ runs: [openRun(100)], mean: 50 });
    ...
    expect(screen.getByText(/평균 응답시간\(50ms\)/)).toBeInTheDocument();
  ...
  it("적용 → onApply(권장)", async () => {
    ...
    setHooks({ runs: [openRun(100)], mean: 250 });
  ...
  it("권장값이 상한(10,000) 초과 → 경고", () => {
    setHooks({ runs: [openRun(100)], mean: 1000 }); // 1s 지연
