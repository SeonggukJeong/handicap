...
export const ko = {
  ...
  editor: {
    ...
    importYaml: "파일에서 열기",
    importYamlAria: "파일에서 열기 — YAML 파일을 선택해 시나리오를 불러옵니다",
    exportYaml: "파일로 저장",
    exportYamlAria: "파일로 저장 — 현재 시나리오를 YAML 파일로 저장합니다",
    ...
    genFieldFormatCustom: (name: string) => `${name} 형식 문자열`,
    genFormatCustomOption: "직접 입력…",
    genFieldOffset: (name: string) => `${name} 오프셋`,
    genFieldTz: (name: string) => `${name} 타임존`,
    genFieldMin: (name: string) => `${name} 최소`,
    ...
    // ── 순차 검증 결과 (R13) ──
    seqResultTitle: "순차 검증 결과",
    seqResultAria: "순차 검증 결과",
    seqRowCount: (n: number) => `${n}행`,
    seqRowLabel: (n: number) => `행 ${n}`,
    seqTruncated: (requested: number, done: number) =>
      `상한 도달로 ${requested}행 중 ${done}행만 실행됨`,
    seqRowTruncated: "이 행은 상한 도달로 중간에 잘렸습니다",
    ...
    // ── test-run 데이터셋 (R11/R14/R15, ADR-0047) ──
    dsSectionTitle: "데이터셋 (선택)",
    dsSectionAria: "테스트 데이터셋 구성",
    dsPickLabel: "데이터셋",
    dsPickNone: "— 선택 안 함 —",
    dsModeSingle: "특정 행",
    dsModeSeq: "순차 검증",
    dsRowNumLabel: "행 번호",
    dsStartRowLabel: "시작 행",
    ...
    dsMappingEdit: "매핑 편집",
    dsMappingAdd: "+ 매핑 추가",
    dsMappingRemoveAria: (i: number) => `매핑 삭제 ${i}`,
    dsMappingColAria: (i: number) => `매핑 컬럼 ${i}`,
    dsMappingVarAria: (i: number) => `매핑 변수 ${i}`,
    dsSummary: (name: string, mode: string) => `${name} · ${mode}`,
    dsBudgetHint: (expected: number, max: number) =>
      `예상 요청 수 ${expected}개가 최대 요청 수 ${max}개를 넘어 일부 행만 실행될 수 있습니다`,
  },
  templates: {
  ...
  insightActions: {
    ...
    load_gen_saturated:
      "동시 실행 수(max_in_flight)를 늘려 다시 실행하세요. 에러·지연이 함께 높으면 대상 서버 한계일 수 있어요.",
  },
  ...
  errorOnset: {
    ...
    refused:
      "연결이 거부됐어요(connection refused) — 서비스가 내려갔거나 포트·주소가 잘못됐을 가능성이 있어요. 대상 주소와 서버 프로세스 상태를 확인하세요.",
    generic:
